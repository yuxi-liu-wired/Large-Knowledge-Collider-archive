...
public class LarkcTest {
	...
	public static void startLarKC() {
		...
		try {
			// Test LarKC without CYC.
			Larkc.start();

			// Test LarKC using CYC.
			// Larkc.start(true);
		} catch (OutOfMemoryError e) {
			logger.error("Error starting LarKC platform.");
			logger.error("An out of memory error occured. Try adding -Xmx512M (or higher) to your VM parameters.");
			e.printStackTrace();
			return;
		} catch (RuntimeException e) {
			logger.error("Runtime error while starting the LarKC platform.");
			logger.error("Exception: {}, Message: {}", e.getClass(),
