...
public final class operation_communication extends SubLTranslatedFile {
	...
	/**
	 * Is this image allowed to transmit. As of now, 4/24/98, there are only 3
	...
	/**
	 * Boolean: If T, then this image can not transmit operations. This
	...
	public static final SubLObject dont_record_operations_locally() {
		{
			...
			return system_parameters.$dont_record_operations_locally$
	...
	public static final SubLObject set_receiving_remote_operationsP(
	...
	public static final SubLObject set_process_auxiliary_operationsP(
			SubLObject v_boolean) {
	...
	/**
	 * The number of successfully processed operations generated by a different
	...
	/**
	 * The number of operations that have been processed and recorded locally by
	...
	public static final SubLObject inc_total_local_operations_recorded() {
		$total_local_operations_recorded$
				.setGlobalValue(Numbers.add(
						$total_local_operations_recorded$.getGlobalValue(),
						ONE_INTEGER));
		return $total_local_operations_recorded$.getGlobalValue();
	}

	/**
	 * The number of local operations that have been transmitted to the master
	...
	/**
	 * The number of operations that have been read from the master transcript
	 ...
	 * @return NIL
	 * */
	@SubL(source = "cycl/operation-communication.lisp", position = 53481)
	public static final SubLObject halt_cyc_image(SubLObject reason) {
		if ((reason == UNPROVIDED)) {
			reason = $str127$done;
		}
		Errors.warn($str128$Halting_Cyc_image____A, reason);
		agenda.halt_agenda(UNPROVIDED);
		if ((NIL != control_vars.kb_loaded())) {
			save_transcript_ops();
			transcript_utilities.mark_local_transcript($str129$QUIT);
	...
	public static final SubLObject map_cyclist_name_to_cyclist_term(
			SubLObject name) {
		{
			SubLObject cyclist_term = (name.isString() ? ((SubLObject) fi
					.fi_find_int(name))
					: ((NIL != forts.valid_fortP(name)) ? ((SubLObject) name)
							: NIL));
			if (((NIL != cyclist_term) && (NIL != kb_accessors
					.cyclistP(cyclist_term)))) {
				return cyclist_term;
			}
		}
	...
	/**
	 * Return the current KB version, i.e. both KB number and number of master
	...
	public static final SubLObject kb_version_string() {
		if (read_master_transcript_op_number().numG(ZERO_INTEGER)) {
			return PrintLow.format(NIL, $str51$_a__a, control_vars.kb_loaded(),
	...
	public static final SubLObject set_communication_mode_internal(
			SubLObject mode) {
	...
	public static final SubLObject set_communication_mode(SubLObject keyword) {
		...
		if ((communication_mode() != keyword)) {
			...
			if (((NIL != control_vars.$init_file_loadedP$.getGlobalValue()) && (NIL != transcript_utilities
					.local_transcript()))) {
				save_transcript_ops();
			}
			{
				...
				if (pcase_var.eql($kw3$TRANSMIT_AND_RECEIVE)) {
					Errors.handleMissingMethodError("This call was replaced for LarKC purposes. Originally a method was called. Refer to number 12972");
				} else if (pcase_var.eql($kw5$TRANSMIT_ONLY)) {
					Errors.handleMissingMethodError("This call was replaced for LarKC purposes. Originally a method was called. Refer to number 12973");
				} else if (pcase_var.eql($kw6$DEAF)) {
					set_to_deaf();
				} else if (pcase_var.eql($kw56$DEAD)) {
					Errors.handleMissingMethodError("This call was replaced for LarKC purposes. Originally a method was called. Refer to number 12968");
				} else if (pcase_var.eql($kw57$DEAD_RECEIVE)) {
					Errors.handleMissingMethodError("This call was replaced for LarKC purposes. Originally a method was called. Refer to number 12969");
				} else {
					Errors.handleMissingMethodError("This call was replaced for LarKC purposes. Originally a method was called. Refer to number 12970");
				}
			}
		}
	...
	/**
	 * Is the change in communication-mode going to change the way processed
	...
	public static final SubLObject transmitting_changeP(SubLObject new_mode) {
		if (get_transmit_state(new_mode).equal(
				get_transmit_state(communication_mode()))) {
			return NIL;
		} else {
			return T;
		}
	...
	/**
	 * Is the change in communication-mode going to change the receive part of
	 * the mode?
	 */
	...
	public static final SubLObject receiving_changeP(SubLObject new_mode) {
		if (get_receive_state(new_mode).equal(
				get_receive_state(communication_mode()))) {
			return NIL;
		} else {
			return T;
		}
	...
	public static final SubLObject transmittingP() {
		if ((communication_mode().equal($kw5$TRANSMIT_ONLY) || communication_mode()
				.equal($kw3$TRANSMIT_AND_RECEIVE))) {
			return T;
		} else {
			return NIL;
		}
	...
	public static final SubLObject storingP() {
		if ((communication_mode().equal($kw6$DEAF) || communication_mode()
				.equal($kw4$RECEIVE_ONLY))) {
			return T;
		} else {
			return NIL;
		}
	...
	public static final SubLObject get_transmit_state(SubLObject mode) {
		...
		}
		{
			...
			if (((mode == $kw3$TRANSMIT_AND_RECEIVE) || (mode == $kw5$TRANSMIT_ONLY))) {
				state = $str64$transmit;
			} else if (((mode == $kw6$DEAF) || (mode == $kw4$RECEIVE_ONLY))) {
				state = $str65$store;
			} else if (((mode == $kw56$DEAD) || (mode == $kw57$DEAD_RECEIVE))) {
				state = $str66$none;
			} else {
				state = NIL;
			}
	...
	public static final SubLObject get_receive_state(SubLObject mode) {
		...
		}
		{
			...
			if (((mode == $kw3$TRANSMIT_AND_RECEIVE)
					|| (mode == $kw4$RECEIVE_ONLY) || (mode == $kw57$DEAD_RECEIVE))) {
				state = $str67$yes;
			} else if (((mode == $kw6$DEAF) || (mode == $kw5$TRANSMIT_ONLY) || (mode == $kw56$DEAD))) {
				state = $str68$no;
			} else {
				state = NIL;
			}
	...
	public static final SubLObject save_transcript_ops() {
		{
			final SubLThread thread = SubLProcess.currentSubLThread();
			if ((NIL != operation_queues.$hl_transcripts_enabledP$
					.getDynamicValue(thread))) {
				Errors.handleMissingMethodError("This call was replaced for LarKC purposes. Originally a method was called. Refer to number 12960");
			}
			if ((NIL == operation_queues.transcript_queue_empty())) {
				{
					SubLObject lock = $save_transcript_ops_lock$
							.getDynamicValue(thread);
	...
	public static final SubLObject save_transcript_ops_internal(
			SubLObject transcript_known_to_exist) {
		...
		}
		{
			final SubLThread thread = SubLProcess.currentSubLThread();
			{
				SubLObject local_transcript = transcript_utilities
						.local_transcript();
				SubLObject direction = ((NIL != Filesys
						.probe_file(local_transcript)) ? ((SubLObject) $kw73$APPEND)
						: $kw74$OUTPUT);
				...
				try {
					{
						...
						try {
							stream_macros.$stream_requires_locking$.bind(NIL,
									thread);
							stream = compatibility.open_text(local_transcript,
									direction);
						} finally {
					...
					}
					{
						...
						if (stream_2.isStream()) {
							{
								SubLObject _prev_bind_0 = print_high.$print_level$
										.currentBinding(thread);
								SubLObject _prev_bind_1 = print_high.$print_length$
										.currentBinding(thread);
								try {
									...
									{
										SubLObject op = NIL;
										for (; (NIL == operation_queues
												.transcript_queue_empty());) {
											op = operation_queues
													.transcript_queue_dequeue();
											if (((NIL == dont_record_operations_locally()) && (NIL != storingP()))) {
												PrintLow.format(stream_2,
														$str76$_s__, op);
												operation_queues
														.local_operation_storage_queue_enqueue(op);
												inc_total_local_operations_recorded();
											}
											if (((NIL != allow_transmitting()) && (NIL != transmittingP()))) {
												Errors.handleMissingMethodError("This call was replaced for LarKC purposes. Originally a method was called. Refer to number 22952");
												Errors.handleMissingMethodError("This call was replaced for LarKC purposes. Originally a method was called. Refer to number 12946");
											}
										}
									}
								} finally {
									print_high.$print_length$.rebind(
											_prev_bind_1, thread);
									print_high.$print_level$.rebind(
											_prev_bind_0, thread);
								}
							}
						}
	...
	public static final SubLObject remote_operation_to_runP() {
		return makeBoolean(((NIL != receiving_remote_operationsP()) && (NIL == Errors
	...
	public static final SubLObject declare_operation_communication_file() {
		declareFunction(myName, "allow_transmitting", "ALLOW-TRANSMITTING", 0,
				0, false);
		// declareFunction(myName, "set_allow_transmitting",
		// "SET-ALLOW-TRANSMITTING", 1, 0, false);
		// declareFunction(myName, "kb_full_transcript_loaded",
		// "KB-FULL-TRANSCRIPT-LOADED", 0, 0, false);
		// declareFunction(myName, "set_kb_full_transcript_loaded",
		// "SET-KB-FULL-TRANSCRIPT-LOADED", 1, 0, false);
		// declareFunction(myName, "experimental_image", "EXPERIMENTAL-IMAGE",
		// 0, 0, false);
		// declareFunction(myName, "set_experimental_image",
		// "SET-EXPERIMENTAL-IMAGE", 1, 0, false);
		// declareFunction(myName, "read_transcript_op_limit",
		// "READ-TRANSCRIPT-OP-LIMIT", 0, 0, false);
		// declareFunction(myName, "set_read_transcript_op_limit",
		// "SET-READ-TRANSCRIPT-OP-LIMIT", 1, 0, false);
		declareFunction(myName, "dont_record_operations_locally",
				"DONT-RECORD-OPERATIONS-LOCALLY", 0, 0, false);
		// declareFunction(myName, "set_dont_record_operations_locally",
		// "SET-DONT-RECORD-OPERATIONS-LOCALLY", 1, 0, false);
		declareFunction(myName, "receiving_remote_operationsP",
				"RECEIVING-REMOTE-OPERATIONS?", 0, 0, false);
		declareFunction(myName, "set_receiving_remote_operationsP",
				"SET-RECEIVING-REMOTE-OPERATIONS?", 1, 0, false);
		declareFunction(myName, "process_local_operationsP",
				"PROCESS-LOCAL-OPERATIONS?", 0, 0, false);
		// declareFunction(myName, "set_process_local_operationsP",
		// "SET-PROCESS-LOCAL-OPERATIONS?", 1, 0, false);
		declareFunction(myName, "process_auxiliary_operationsP",
				"PROCESS-AUXILIARY-OPERATIONS?", 0, 0, false);
		declareFunction(myName, "set_process_auxiliary_operationsP",
				"SET-PROCESS-AUXILIARY-OPERATIONS?", 1, 0, false);
		declareFunction(myName, "saving_operationsP", "SAVING-OPERATIONS?", 0,
				0, false);
		declareFunction(myName, "set_saving_operationsP",
				"SET-SAVING-OPERATIONS?", 1, 0, false);
		// declareFunction(myName, "total_remote_operations_run",
		// "TOTAL-REMOTE-OPERATIONS-RUN", 0, 0, false);
		// declareFunction(myName, "set_total_remote_operations_run",
		// "SET-TOTAL-REMOTE-OPERATIONS-RUN", 1, 0, false);
		// declareFunction(myName, "total_auxiliary_operations_run",
		// "TOTAL-AUXILIARY-OPERATIONS-RUN", 0, 0, false);
		// declareFunction(myName, "inc_total_auxiliary_operations_run",
		// "INC-TOTAL-AUXILIARY-OPERATIONS-RUN", 0, 0, false);
		// declareFunction(myName, "total_local_operations_recorded",
		// "TOTAL-LOCAL-OPERATIONS-RECORDED", 0, 0, false);
		declareFunction(myName, "inc_total_local_operations_recorded",
				"INC-TOTAL-LOCAL-OPERATIONS-RECORDED", 0, 0, false);
		// declareFunction(myName, "total_local_operations_transmitted",
		// "TOTAL-LOCAL-OPERATIONS-TRANSMITTED", 0, 0, false);
		// declareFunction(myName, "inc_total_local_operations_transmitted",
		// "INC-TOTAL-LOCAL-OPERATIONS-TRANSMITTED", 0, 0, false);
		declareFunction(myName, "read_master_transcript_op_number",
				"READ-MASTER-TRANSCRIPT-OP-NUMBER", 0, 0, false);
		// declareFunction(myName, "set_read_master_transcript_op_number",
		// "SET-READ-MASTER-TRANSCRIPT-OP-NUMBER", 1, 0, false);
		// declareFunction(myName,
		// "total_master_transcript_operations_processed",
		// "TOTAL-MASTER-TRANSCRIPT-OPERATIONS-PROCESSED", 0, 0, false);
		// declareFunction(myName,
		// "set_total_master_transcript_operations_processed",
		// "SET-TOTAL-MASTER-TRANSCRIPT-OPERATIONS-PROCESSED", 1, 0, false);
		declareFunction(myName, "the_cyclist", "THE-CYCLIST", 0, 0, false);
		declareFunction(myName, "set_the_cyclist", "SET-THE-CYCLIST", 0, 1,
				false);
		declareFunction(myName, "map_cyclist_name_to_cyclist_term",
				"MAP-CYCLIST-NAME-TO-CYCLIST-TERM", 1, 0, false);
		// declareFunction(myName, "the_cyclist_is_guestP",
		// "THE-CYCLIST-IS-GUEST?", 0, 0, false);
		// declareFunction(myName, "image_requires_authenticationP",
		// "IMAGE-REQUIRES-AUTHENTICATION?", 0, 0, false);
		// declareFunction(myName, "authenticate_the_cyclist",
		// "AUTHENTICATE-THE-CYCLIST", 2, 1, false);
		// declareFunction(myName, "lookup_password_for_cyclist_from_kb",
		// "LOOKUP-PASSWORD-FOR-CYCLIST-FROM-KB", 1, 1, false);
		// declareFunction(myName, "get_stored_password_for_cyclist_from_kb",
		// "GET-STORED-PASSWORD-FOR-CYCLIST-FROM-KB", 1, 0, false);
		// declareFunction(myName, "specify_authentication_info_for_user",
		// "SPECIFY-AUTHENTICATION-INFO-FOR-USER", 2, 1, false);
		// declareFunction(myName, "assert_stored_password_for_cyclist_in_kb",
		// "ASSERT-STORED-PASSWORD-FOR-CYCLIST-IN-KB", 2, 0, false);
		// declareFunction(myName, "unassert_stored_password_for_cyclist_in_kb",
		// "UNASSERT-STORED-PASSWORD-FOR-CYCLIST-IN-KB", 1, 0, false);
		// declareFunction(myName, "kb_op_number", "KB-OP-NUMBER", 0, 0, false);
		declareFunction(myName, "kb_version_string", "KB-VERSION-STRING", 0, 0,
				false);
		// declareFunction(myName, "remote_ops_processed",
		// "REMOTE-OPS-PROCESSED", 0, 0, false);
		// declareFunction(myName, "available_communication_modes",
		// "AVAILABLE-COMMUNICATION-MODES", 0, 1, false);
		// declareFunction(myName, "communication_modeP", "COMMUNICATION-MODE?",
		// 1, 0, false);
		// declareFunction(myName, "communication_mode_name",
		// "COMMUNICATION-MODE-NAME", 1, 0, false);
		declareFunction(myName, "communication_mode", "COMMUNICATION-MODE", 0,
				0, false);
		declareFunction(myName, "set_communication_mode_internal",
				"SET-COMMUNICATION-MODE-INTERNAL", 1, 0, false);
		declareFunction(myName, "set_communication_mode",
				"SET-COMMUNICATION-MODE", 0, 1, false);
		// declareFunction(myName, "transmit_state_p", "TRANSMIT-STATE-P", 1, 0,
		// false);
		// declareFunction(myName, "receive_state_p", "RECEIVE-STATE-P", 1, 0,
		// false);
		// declareFunction(myName, "set_communication_state",
		// "SET-COMMUNICATION-STATE", 2, 0, false);
		// declareFunction(myName, "set_transmit_state", "SET-TRANSMIT-STATE",
		// 1, 0, false);
		// declareFunction(myName, "set_receive_state", "SET-RECEIVE-STATE", 1,
		// 0, false);
		// declareFunction(myName, "set_to_transmitting", "SET-TO-TRANSMITTING",
		// 0, 0, false);
		declareFunction(myName, "set_to_storing", "SET-TO-STORING", 0, 0, false);
		// declareFunction(myName, "set_to_no_record", "SET-TO-NO-RECORD", 0, 0,
		// false);
		// declareFunction(myName, "set_to_receiving", "SET-TO-RECEIVING", 0, 0,
		// false);
		declareFunction(myName, "set_to_not_receiving", "SET-TO-NOT-RECEIVING",
				0, 0, false);
		declareFunction(myName, "transmitting_changeP", "TRANSMITTING-CHANGE?",
				1, 0, false);
		declareFunction(myName, "receiving_changeP", "RECEIVING-CHANGE?", 1, 0,
				false);
		// declareFunction(myName, "set_to_receive_only", "SET-TO-RECEIVE-ONLY",
		// 0, 0, false);
		declareFunction(myName, "set_to_deaf", "SET-TO-DEAF", 0, 0, false);
		// declareFunction(myName, "set_to_dead", "SET-TO-DEAD", 0, 0, false);
		// declareFunction(myName, "set_to_transmit_and_receive",
		// "SET-TO-TRANSMIT-AND-RECEIVE", 0, 0, false);
		// declareFunction(myName, "set_to_transmit_only",
		// "SET-TO-TRANSMIT-ONLY", 0, 0, false);
		// declareFunction(myName, "set_to_dead_receive", "SET-TO-DEAD-RECEIVE",
		// 0, 0, false);
		declareFunction(myName, "transmittingP", "TRANSMITTING?", 0, 0, false);
		declareFunction(myName, "storingP", "STORING?", 0, 0, false);
		// declareFunction(myName, "not_recordingP", "NOT-RECORDING?", 0, 0,
		// false);
		// declareFunction(myName, "receivingP", "RECEIVING?", 0, 0, false);
		declareFunction(myName, "get_transmit_state", "GET-TRANSMIT-STATE", 0,
				1, false);
		declareFunction(myName, "get_receive_state", "GET-RECEIVE-STATE", 0, 1,
				false);
		// declareFunction(myName, "get_communication_state",
		// "GET-COMMUNICATION-STATE", 2, 0, false);
		// declareFunction(myName, "valid_transcript_expression",
		// "VALID-TRANSCRIPT-EXPRESSION", 1, 0, false);
		// declareFunction(myName, "operation_from_meP", "OPERATION-FROM-ME?",
		// 1, 0, false);
		// declareFunction(myName, "writing_transcriptsP",
		// "WRITING-TRANSCRIPTS?", 0, 0, false);
		// declareFunction(myName, "reading_transcriptsP",
		// "READING-TRANSCRIPTS?", 0, 0, false);
		// declareFunction(myName, "clear_stored_operations",
		// "CLEAR-STORED-OPERATIONS", 0, 0, false);
		// declareFunction(myName, "send_stored_operations",
		// "SEND-STORED-OPERATIONS", 0, 0, false);
		// declareFunction(myName, "fi_save_ops", "FI-SAVE-OPS", 0, 0, false);
		// declareFunction(myName, "send_local_transcript",
		// "SEND-LOCAL-TRANSCRIPT", 0, 0, false);
		// declareFunction(myName, "possibly_save_transcript_ops",
		// "POSSIBLY-SAVE-TRANSCRIPT-OPS", 0, 0, false);
		declareFunction(myName, "save_transcript_ops", "SAVE-TRANSCRIPT-OPS",
				0, 0, false);
		declareFunction(myName, "save_transcript_ops_internal",
				"SAVE-TRANSCRIPT-OPS-INTERNAL", 0, 1, false);
		// declareFunction(myName, "possibly_save_hl_transcript_ops",
		// "POSSIBLY-SAVE-HL-TRANSCRIPT-OPS", 0, 0, false);
		// declareFunction(myName, "save_hl_transcript_ops",
		// "SAVE-HL-TRANSCRIPT-OPS", 0, 0, false);
		// declareFunction(myName, "save_hl_transcript_ops_internal",
		// "SAVE-HL-TRANSCRIPT-OPS-INTERNAL", 0, 0, false);
		// declareFunction(myName, "close_kb", "CLOSE-KB", 2, 0, false);
		// declareFunction(myName, "read_one_transcript_operation",
		// "READ-ONE-TRANSCRIPT-OPERATION", 1, 0, false);
		// declareFunction(myName, "backup_transcript_stream",
		// "BACKUP-TRANSCRIPT-STREAM", 2, 0, false);
		// declareFunction(myName, "load_transcript_exprs",
		// "LOAD-TRANSCRIPT-EXPRS", 0, 0, false);
		// declareFunction(myName, "load_transcript_exprs_internal",
		// "LOAD-TRANSCRIPT-EXPRS-INTERNAL", 0, 0, false);
		declareFunction(myName, "remote_operation_to_runP",
				"REMOTE-OPERATION-TO-RUN?", 0, 0, false);
		// declareFunction(myName, "master_transcript_catch_up",
		// "MASTER-TRANSCRIPT-CATCH-UP", 0, 0, false);
		// declareFunction(myName, "run_random_transcript_op",
		// "RUN-RANDOM-TRANSCRIPT-OP", 1, 0, false);
		// declareFunction(myName, "load_transcript_file",
		// "LOAD-TRANSCRIPT-FILE", 1, 4, false);
		// declareFunction(myName, "server_summary", "SERVER-SUMMARY", 0, 1,
		// false);
		// declareFunction(myName, "write_cyc_image", "WRITE-CYC-IMAGE", 1, 1,
		// false);
		// declareFunction(myName, "make_cyc_image_filename",
		// "MAKE-CYC-IMAGE-FILENAME", 0, 1, false);
		// declareFunction(myName, "make_next_cyc_image_filename",
		// "MAKE-NEXT-CYC-IMAGE-FILENAME", 1, 0, false);
		// declareFunction(myName, "halt_cyc_image", "HALT-CYC-IMAGE", 0, 1,
		// false);
		// declareFunction(myName, "get_remote_operations_kb_check_point",
		// "GET-REMOTE-OPERATIONS-KB-CHECK-POINT", 0, 0, false);
		// declareFunction(myName, "set_remote_operations_kb_check_point",
		// "SET-REMOTE-OPERATIONS-KB-CHECK-POINT", 1, 0, false);
		// declareFunction(myName, "clear_remote_operations_kb_check_point",
		// "CLEAR-REMOTE-OPERATIONS-KB-CHECK-POINT", 0, 0, false);
		// declareFunction(myName, "note_kb_check_point_reached",
		// "NOTE-KB-CHECK-POINT-REACHED", 1, 0, false);
		// declareFunction(myName, "transmit_kb_check_point_operation",
		// "TRANSMIT-KB-CHECK-POINT-OPERATION", 1, 0, false);
		// declareFunction(myName, "possibly_roll_to_allow_transmitting",
		// "POSSIBLY-ROLL-TO-ALLOW-TRANSMITTING", 0, 0, false);
		return NIL;
	}

	public static final SubLObject init_operation_communication_file() {
		$all_communication_modes$ = defparameter("*ALL-COMMUNICATION-MODES*",
				$list0);
		$allow_transmitting$ = deflexical(
				"*ALLOW-TRANSMITTING*",
				((NIL != Symbols.boundp($sym1$_ALLOW_TRANSMITTING_)) ? ((SubLObject) $allow_transmitting$
						.getGlobalValue()) : T));
		$kb_full_transcript_loaded$ = deflexical(
				"*KB-FULL-TRANSCRIPT-LOADED*",
				((NIL != Symbols.boundp($sym2$_KB_FULL_TRANSCRIPT_LOADED_)) ? ((SubLObject) $kb_full_transcript_loaded$
						.getGlobalValue()) : NIL));
		$experimental_image$ = defparameter("*EXPERIMENTAL-IMAGE*", NIL);
		$read_transcript_op_limit$ = defparameter("*READ-TRANSCRIPT-OP-LIMIT*",
				$int7$200);
		$receiving_remote_operationsP$ = deflexical(
				"*RECEIVING-REMOTE-OPERATIONS?*",
				((NIL != Symbols.boundp($sym8$_RECEIVING_REMOTE_OPERATIONS__)) ? ((SubLObject) $receiving_remote_operationsP$
						.getGlobalValue()) : T));
		$process_local_operationsP$ = deflexical(
				"*PROCESS-LOCAL-OPERATIONS?*",
				((NIL != Symbols.boundp($sym9$_PROCESS_LOCAL_OPERATIONS__)) ? ((SubLObject) $process_local_operationsP$
						.getGlobalValue()) : T));
		$process_auxiliary_operationsP$ = deflexical(
				"*PROCESS-AUXILIARY-OPERATIONS?*",
				((NIL != Symbols.boundp($sym10$_PROCESS_AUXILIARY_OPERATIONS__)) ? ((SubLObject) $process_auxiliary_operationsP$
						.getGlobalValue()) : T));
		$saving_operationsP$ = deflexical(
				"*SAVING-OPERATIONS?*",
				((NIL != Symbols.boundp($sym11$_SAVING_OPERATIONS__)) ? ((SubLObject) $saving_operationsP$
						.getGlobalValue()) : NIL));
		$total_remote_operations_run$ = deflexical(
				"*TOTAL-REMOTE-OPERATIONS-RUN*",
				((NIL != Symbols.boundp($sym12$_TOTAL_REMOTE_OPERATIONS_RUN_)) ? ((SubLObject) $total_remote_operations_run$
						.getGlobalValue()) : ZERO_INTEGER));
		$total_auxiliary_operations_run$ = deflexical(
				"*TOTAL-AUXILIARY-OPERATIONS-RUN*",
				((NIL != Symbols
						.boundp($sym13$_TOTAL_AUXILIARY_OPERATIONS_RUN_)) ? ((SubLObject) $total_auxiliary_operations_run$
						.getGlobalValue()) : ZERO_INTEGER));
		$total_local_operations_recorded$ = deflexical(
				"*TOTAL-LOCAL-OPERATIONS-RECORDED*",
				((NIL != Symbols
						.boundp($sym14$_TOTAL_LOCAL_OPERATIONS_RECORDED_)) ? ((SubLObject) $total_local_operations_recorded$
						.getGlobalValue()) : ZERO_INTEGER));
		$total_local_operations_transmitted$ = deflexical(
				"*TOTAL-LOCAL-OPERATIONS-TRANSMITTED*",
				((NIL != Symbols
						.boundp($sym15$_TOTAL_LOCAL_OPERATIONS_TRANSMITTED_)) ? ((SubLObject) $total_local_operations_transmitted$
						.getGlobalValue()) : ZERO_INTEGER));
		$read_master_transcript_op_number$ = deflexical(
				"*READ-MASTER-TRANSCRIPT-OP-NUMBER*",
				((NIL != Symbols
						.boundp($sym16$_READ_MASTER_TRANSCRIPT_OP_NUMBER_)) ? ((SubLObject) $read_master_transcript_op_number$
						.getGlobalValue()) : ZERO_INTEGER));
		$total_master_transcript_operations_processed$ = deflexical(
				"*TOTAL-MASTER-TRANSCRIPT-OPERATIONS-PROCESSED*",
				((NIL != Symbols
						.boundp($sym17$_TOTAL_MASTER_TRANSCRIPT_OPERATIONS_PROCESSED_)) ? ((SubLObject) $total_master_transcript_operations_processed$
						.getGlobalValue()) : ZERO_INTEGER));
		$image_requires_authenticationP$ = deflexical(
				"*IMAGE-REQUIRES-AUTHENTICATION?*", NIL);
		$cyclist_authenticating_app$ = defparameter(
				"*CYCLIST-AUTHENTICATING-APP*", $const26$CycBrowser);
		$default_cyclist_authentication_mt$ = defparameter(
				"*DEFAULT-CYCLIST-AUTHENTICATION-MT*", $const34$CyclistsMt);
		$transmit_states$ = deflexical("*TRANSMIT-STATES*", $list58);
		$receive_states$ = deflexical("*RECEIVE-STATES*", $list60);
		$build_request_email$ = deflexical("*BUILD-REQUEST-EMAIL*",
				$str72$build_requests_cyc_com);
		$save_transcript_ops_lock$ = defparameter("*SAVE-TRANSCRIPT-OPS-LOCK*",
				Locks.make_lock($str77$Save_Transcript_OPS_lock));
		$save_hl_transcript_ops_lock$ = defparameter(
				"*SAVE-HL-TRANSCRIPT-OPS-LOCK*",
				Locks.make_lock($str78$Save_Hl_Transcript_OPS_lock));
		$transcript_load_succeeded$ = defvar("*TRANSCRIPT-LOAD-SUCCEEDED*", NIL);
		$transcript_read_attempted$ = deflexical(
				"*TRANSCRIPT-READ-ATTEMPTED*",
				((NIL != Symbols.boundp($sym85$_TRANSCRIPT_READ_ATTEMPTED_)) ? ((SubLObject) $transcript_read_attempted$
						.getGlobalValue()) : NIL));
		$initial_read_transcript_size$ = deflexical(
				"*INITIAL-READ-TRANSCRIPT-SIZE*",
				((NIL != Symbols.boundp($sym86$_INITIAL_READ_TRANSCRIPT_SIZE_)) ? ((SubLObject) $initial_read_transcript_size$
						.getGlobalValue()) : ZERO_INTEGER));
		$remote_operations_kb_check_point$ = deflexical(
				"*REMOTE-OPERATIONS-KB-CHECK-POINT*",
				((NIL != Symbols
						.boundp($sym130$_REMOTE_OPERATIONS_KB_CHECK_POINT_)) ? ((SubLObject) $remote_operations_kb_check_point$
						.getGlobalValue()) : NIL));
		return NIL;
	}
	...
	public static final SubLObject setup_operation_communication_file() {
		...
		subl_macro_promotions
				.declare_defglobal($sym2$_KB_FULL_TRANSCRIPT_LOADED_);
		subl_macro_promotions
				.declare_defglobal($sym8$_RECEIVING_REMOTE_OPERATIONS__);
		subl_macro_promotions
				.declare_defglobal($sym9$_PROCESS_LOCAL_OPERATIONS__);
		subl_macro_promotions
				.declare_defglobal($sym10$_PROCESS_AUXILIARY_OPERATIONS__);
		subl_macro_promotions.declare_defglobal($sym11$_SAVING_OPERATIONS__);
		subl_macro_promotions
				.declare_defglobal($sym12$_TOTAL_REMOTE_OPERATIONS_RUN_);
		subl_macro_promotions
				.declare_defglobal($sym13$_TOTAL_AUXILIARY_OPERATIONS_RUN_);
		subl_macro_promotions
				.declare_defglobal($sym14$_TOTAL_LOCAL_OPERATIONS_RECORDED_);
		subl_macro_promotions
				.declare_defglobal($sym15$_TOTAL_LOCAL_OPERATIONS_TRANSMITTED_);
		subl_macro_promotions
				.declare_defglobal($sym16$_READ_MASTER_TRANSCRIPT_OP_NUMBER_);
		subl_macro_promotions
				.declare_defglobal($sym17$_TOTAL_MASTER_TRANSCRIPT_OPERATIONS_PROCESSED_);
		access_macros.register_external_symbol($sym18$THE_CYCLIST);
		utilities_macros.register_cyc_api_function(
				$sym20$THE_CYCLIST_IS_GUEST_, NIL,
				$str21$Return_T_iff_the_current_CyclIst_, NIL, $list22);
		utilities_macros.register_cyc_api_function(
				$sym23$IMAGE_REQUIRES_AUTHENTICATION_, NIL,
				$str24$_return_non_NIL_iff_this_Cyc_imag, NIL, $list25);
		utilities_macros.register_cyc_api_function(
				$sym29$AUTHENTICATE_THE_CYCLIST, $list30,
				$str31$Ensure_that_the_encrypted_passphr, $list32, $list33);
		utilities_macros.register_cyc_api_function(
				$sym41$SPECIFY_AUTHENTICATION_INFO_FOR_USER, $list42,
				$str43$Store_the_authenitcation_informat, $list44, $list45);
		access_macros.register_external_symbol($sym50$KB_VERSION_STRING);
		subl_macro_promotions
				.declare_defglobal($sym85$_TRANSCRIPT_READ_ATTEMPTED_);
		subl_macro_promotions
				.declare_defglobal($sym86$_INITIAL_READ_TRANSCRIPT_SIZE_);
		subl_macro_promotions
				.declare_defglobal($sym130$_REMOTE_OPERATIONS_KB_CHECK_POINT_);
		return NIL;
	}

	// // Internal Constants

	public static final SubLList $list0 = list(
			cons(makeKeyword("TRANSMIT-AND-RECEIVE"),
					makeString("Sending and Receiving")),
			cons(makeKeyword("RECEIVE-ONLY"),
					makeString("Storing and Receiving")),
			cons(makeKeyword("TRANSMIT-ONLY"), makeString("Sending Only")),
			cons(makeKeyword("DEAD-RECEIVE"),
					makeString("Not Recording but Receiving")),
			cons(makeKeyword("DEAF"), makeString("Storing Only")),
			cons(makeKeyword("DEAD"), makeString("Not Recording or Receiving")));
	...
	public static final SubLObject $const19$Guest = constant_handles
			.reader_make_constant_shell(makeString("Guest"));
	...
	public static final SubLList $list22 = list(list(makeSymbol("LIST"),
			makeSymbol("BOOLEANP")));
	...
	public static final SubLObject $const26$CycBrowser = constant_handles
			.reader_make_constant_shell(makeString("CycBrowser"));
	...
	public static final SubLList $list30 = list(
			makeSymbol("NAME"),
			makeSymbol("ENCRYPTED-PASSWORD"),
			makeSymbol("&OPTIONAL"),
			list(makeSymbol("APPLICATION"),
					makeSymbol("*CYCLIST-AUTHENTICATING-APP*")));
	...
	public static final SubLList $list32 = list(
			list(makeSymbol("NAME"), makeSymbol("STRINGP")),
			list(makeSymbol("ENCRYPTED-PASSWORD"), makeSymbol("STRINGP")),
			list(makeSymbol("APPLICATION"), makeSymbol("FORT-P")));
	public static final SubLList $list33 = list(makeSymbol("FORT-P"));
	public static final SubLObject $const34$CyclistsMt = constant_handles
			.reader_make_constant_shell(makeString("CyclistsMt"));
	public static final SubLObject $const35$UserOfProgramFn = constant_handles
			.reader_make_constant_shell(makeString("UserOfProgramFn"));
	public static final SubLSymbol $sym36$RELEVANT_MT_IS_GENL_MT = makeSymbol("RELEVANT-MT-IS-GENL-MT");
	public static final SubLObject $const37$encryptedPasswordForUser = constant_handles
			.reader_make_constant_shell(makeString("encryptedPasswordForUser"));
	...
	public static final SubLList $list42 = list(
			makeSymbol("CYCLIST"),
			makeSymbol("PASSPHRASE"),
			makeSymbol("&OPTIONAL"),
			list(makeSymbol("APPLICATION"),
					makeSymbol("*CYCLIST-AUTHENTICATING-APP*")));
	...
	public static final SubLList $list44 = list(
			list(makeSymbol("CYCLIST"), makeSymbol("FORT-P")),
			list(makeSymbol("PASSPHRASE"), makeSymbol("STRINGP")));
	...
	public static final SubLList $list58 = list(makeString("transmit"),
			makeString("store"), makeString("none"));
	public static final SubLSymbol $sym59$STRING_EQUAL = makeSymbol("STRING-EQUAL");
	public static final SubLList $list60 = list(makeString("yes"),
			makeString("no"));
	...
	public static final SubLList $list116 = list(makeSymbol("SYSTEM-NUMBER"),
			makeSymbol("&REST"), makeSymbol("PATCH-NUMBERS"));
