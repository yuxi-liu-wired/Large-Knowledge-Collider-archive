...
public final class system_parameters extends SubLTranslatedFile {
	...
	public static final SubLObject register_system_parameter(SubLObject name,
			SubLObject v_default, SubLObject type, SubLObject description) {
		if ((NIL == conses_high.member(type,
				$valid_system_parameter_types$.getGlobalValue(), UNPROVIDED,
				UNPROVIDED))) {
			Errors.warn($str3$_S__S_has_an_unknown_type__S_,
					$sym4$DEFINE_SYSTEM_PARAMETER, name, type);
		}
		remove_system_parameter(name);
		$system_parameters$.setGlobalValue(cons(
				list(name, v_default, type, description),
				$system_parameters$.getGlobalValue()));
		return name;
	}

	/**
	 * @param NAME
	 *            : symbolp Remove NAME from the system parameters.
	 */
	...
	public static final SubLObject remove_system_parameter(SubLObject name) {
		...
		$system_parameters$.setGlobalValue(Sequences.delete(name,
				$system_parameters$.getGlobalValue(),
				Symbols.symbol_function(EQ),
				Symbols.symbol_function($sym6$CAR), UNPROVIDED, UNPROVIDED,
				UNPROVIDED));
		return NIL;
	}

	@SubL(source = "cycl/system-parameters.lisp", position = 2664)
	public static final SubLObject system_parameter_value_unset_p(
			SubLObject value) {
	...
	public static final SubLObject check_system_parameters() {
		{
			...
			for (entry = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
					...
					if ((NIL == current)) {
						if ((NIL == Symbols.boundp(symbol))) {
							Errors.warn(
									...
									symbol);
						} else if ((NIL != system_parameter_value_unset_p(Symbols
								.symbol_value(symbol)))) {
							Errors.warn(
						...
						} else {
							{
								...
								if (pcase_var.eql($sym16$T_OR_NIL)) {
									...
									okP = makeBoolean(((NIL == value) || value
											.isString()));
	...
	public static final SubLObject load_system_parameters() {
		{
			SubLObject filename = file_utilities.cyc_home_filename($list26,
					$str27$parameters, $str28$lisp);
	...
	public static final SubLObject read_parameter_form(SubLObject stream) {
		{
			final SubLThread thread = SubLProcess.currentSubLThread();
			{
				...
				{
					SubLObject _prev_bind_0 = reader.$read_eval$
							.currentBinding(thread);
					try {
						reader.$read_eval$.bind(NIL, thread);
						parameter_form = reader.read(stream, NIL, $kw45$EOF,
								UNPROVIDED);
					} finally {
			...
			current = current.rest();
			{
				...
				if (pcase_var.eql($sym48$IN_PACKAGE)) {
				...
				} else if (pcase_var.eql($sym50$CSETQ)) {
					{
						...
						cdestructuring_bind.destructuring_bind_must_consp(
								current_6, datum_5, $list51);
						...
						current_6 = current_6.rest();
						if ((NIL == current_6)) {
							if ((NIL != conses_high.member(symbol,
									$system_parameters$.getGlobalValue(),
									Symbols.symbol_function(EQL),
									Symbols.symbol_function($sym2$FIRST)))) {
								Symbols.set(symbol,
										evaluate_parameter_value(value));
							}
						} else {
							cdestructuring_bind.cdestructuring_bind_error(
									datum_5, $list51);
						}
					}
	 ...
	 * written to the master transcript. If T, then the image does not write to
	 * a local transcript file, and will write to the master transcript when it
	 * is set to transmit operations. This allows an image that is standalone,
	...
	/**
	 * Possible values -- :TRANSMIT-AND-RECEIVE, :RECEIVE-ONLY, :ISOLATED,
	 * :DEAF, :DEAD. This is the communication mode the cyc image should get
	 * initialized to at startup.
	 */
	 ...
	 * startup, but can be enabled later by the user. If T, the agenda is
	 * enabled at startup.
	 */
	 ...
	 * API (application program interface) service from *base-tcp-port*.
	 */
	...
	/**
	 * Possible values: T, NIL. IF T, then API functions can access host
	 * services including the file system and outbound tcp connections. The most
	...
	/**
	 * Possible values: T, NIL. IF T, then writing to the master transcript will
	 * be controlled by the Cyc Transcript Server, which will need to be
	 * installed at your site. You only need to set this to T if you are running
	 * multiple instances of Cyc. If NIL, then Cyc will read and write to the
	 * master transcript without regard to other processes doing the same.
	 */
	@SubL(source = "cycl/system-parameters.lisp", position = 13882)
	public static SubLSymbol $use_transcript_server$ = null;

	/**
	 * Possible values: NIL or a string. This parameter is only used if
	 * *USE-TRANSCRIPT-SERVER* is T. If so, then this parameter should be set to
	 * the name of the host offering the cyc-serializer service.
	 */
	 ...
	 * USE-TRANSCRIPT-SERVER* is T. If so, then this parameter should be set to
	 * the port number of the cyc-serializer read service.
	 */
	 ...
	 * modifications to the knowledge base. If T, any user is allowed to modify
	 * the knowledge base.
	 */
	...
	/**
	 * Possible values: The name of a constant representing a Cyclist. This is
	 * the default Cyclist initially logged into the system.
	 */
	...
	/**
	 * Possible values: One of the symbols :CYCORP or :UNKNOWN. If the execution
	 * context is set to :CYCORP, then the CYC image can assume that it is
	 * running in Cycorp's development environment and make strong assumptions
	 * about setup and infrastructure.
	 */
	...
	public static final SubLObject declare_system_parameters_file() {
		declareFunction(myName, "all_system_parameters",
				"ALL-SYSTEM-PARAMETERS", 0, 0, false);
		declareFunction(myName, "register_system_parameter",
				"REGISTER-SYSTEM-PARAMETER", 4, 0, false);
		declareFunction(myName, "remove_system_parameter",
				"REMOVE-SYSTEM-PARAMETER", 1, 0, false);
		declareMacro(myName, "define_system_parameter",
				"DEFINE-SYSTEM-PARAMETER");
		declareFunction(myName, "system_parameter_value_unset_p",
				"SYSTEM-PARAMETER-VALUE-UNSET-P", 1, 0, false);
		declareFunction(myName, "check_system_parameters",
				"CHECK-SYSTEM-PARAMETERS", 0, 0, false);
		declareFunction(myName, "alphanumericize_string",
				"ALPHANUMERICIZE-STRING", 1, 0, false);
		declareFunction(myName, "setup_system_parameters",
				"SETUP-SYSTEM-PARAMETERS", 1, 1, false);
		declareFunction(myName, "load_system_parameters",
				"LOAD-SYSTEM-PARAMETERS", 0, 0, false);
		declareFunction(myName, "read_parameter_form", "READ-PARAMETER-FORM",
				1, 0, false);
		declareFunction(myName, "evaluate_parameter_form",
				"EVALUATE-PARAMETER-FORM", 1, 0, false);
		declareFunction(myName, "evaluate_parameter_value",
				"EVALUATE-PARAMETER-VALUE", 1, 0, false);
		return NIL;
	}

	public static final SubLObject init_system_parameters_file() {
		$valid_system_parameter_types$ = deflexical(
				"*VALID-SYSTEM-PARAMETER-TYPES*", $list0);
		$system_parameters$ = deflexical(
				"*SYSTEM-PARAMETERS*",
				((NIL != Symbols.boundp($sym1$_SYSTEM_PARAMETERS_)) ? ((SubLObject) $system_parameters$
						.getGlobalValue()) : NIL));
		$auto_continue_transcript_problems$ = defvar(
				"*AUTO-CONTINUE-TRANSCRIPT-PROBLEMS*", $kw12$UNSET);
		$continue_agenda_on_error$ = defvar("*CONTINUE-AGENDA-ON-ERROR*",
				$kw12$UNSET);
		$suspend_sbhl_type_checkingP$ = defvar("*SUSPEND-SBHL-TYPE-CHECKING?*",
				$kw12$UNSET);
		$really_count_transcript_ops$ = defvar("*REALLY-COUNT-TRANSCRIPT-OPS*",
				$kw12$UNSET);
		$dont_record_operations_locally$ = defvar(
				"*DONT-RECORD-OPERATIONS-LOCALLY*", $kw12$UNSET);
		$startup_communication_mode$ = defvar("*STARTUP-COMMUNICATION-MODE*",
				$kw12$UNSET);
		$start_agenda_at_startupP$ = defvar("*START-AGENDA-AT-STARTUP?*",
				$kw12$UNSET);
		...
		$permit_api_host_access$ = defvar("*PERMIT-API-HOST-ACCESS*",
				$kw12$UNSET);
		$use_transcript_server$ = defvar("*USE-TRANSCRIPT-SERVER*", $kw12$UNSET);
		$master_transcript_lock_host$ = defvar("*MASTER-TRANSCRIPT-LOCK-HOST*",
				$kw12$UNSET);
		$master_transcript_server_port$ = defvar(
				"*MASTER-TRANSCRIPT-SERVER-PORT*", $kw12$UNSET);
		...
		$html_javascript_directory$ = defvar("*HTML-JAVASCRIPT-DIRECTORY*",
				$kw12$UNSET);
	...
	public static final SubLObject setup_system_parameters_file() {
		...
		register_system_parameter($sym53$_AUTO_CONTINUE_TRANSCRIPT_PROBLEMS_,
				T, $sym16$T_OR_NIL, $str54$Possible_values__NIL__T___If_NIL_);
		register_system_parameter($sym55$_CONTINUE_AGENDA_ON_ERROR_, T,
				$sym16$T_OR_NIL, $str56$Possible_values__NIL__T___If_NIL_);
		register_system_parameter($sym57$_SUSPEND_SBHL_TYPE_CHECKING__, NIL,
				$sym16$T_OR_NIL, $str58$Possible_values__T__NIL___Type_ch);
		register_system_parameter($sym59$_REALLY_COUNT_TRANSCRIPT_OPS_, NIL,
				$sym16$T_OR_NIL, $str60$Possible_values__T__NIL___If_NIL_);
		register_system_parameter($sym61$_DONT_RECORD_OPERATIONS_LOCALLY_, NIL,
				$sym16$T_OR_NIL, $str62$Possible_values__NIL__T___If_NIL_);
		register_system_parameter($sym63$_STARTUP_COMMUNICATION_MODE_,
				$kw64$DEAF, $sym21$SYMBOL,
				$str65$Possible_values_____TRANSMIT_AND_);
		register_system_parameter($sym66$_START_AGENDA_AT_STARTUP__, T,
				$sym16$T_OR_NIL, $str67$Possible_values__T__NIL___If_NIL_);
		register_system_parameter($sym68$_BASE_TCP_PORT_, $int69$3600,
				$sym20$INTEGER, $str70$The_base_port_offset_for_all_the_);
		register_system_parameter($sym71$_FI_PORT_OFFSET_, ONE_INTEGER,
				$sym20$INTEGER, $str72$Possible_values__A_number___This_);
		register_system_parameter($sym73$_CFASL_PORT_OFFSET_, FOURTEEN_INTEGER,
				$sym20$INTEGER, $str74$Possible_values__A_number___This_);
		register_system_parameter($sym75$_TCP_LOCALHOST_ONLY__, NIL,
				$sym16$T_OR_NIL, $str76$Possible_values__T__NIL___IF_NIL_);
		register_system_parameter($sym77$_PERMIT_API_HOST_ACCESS_, T,
				$sym16$T_OR_NIL, $str78$Possible_values__T__NIL___IF_T__t);
		register_system_parameter($sym79$_USE_TRANSCRIPT_SERVER_, NIL,
				$sym16$T_OR_NIL, $str80$Possible_values__T__NIL___IF_T__t);
		register_system_parameter($sym81$_MASTER_TRANSCRIPT_LOCK_HOST_, NIL,
				$sym17$NIL_OR_STRING, $str82$Possible_values__NIL_or_a_string_);
		register_system_parameter($sym83$_MASTER_TRANSCRIPT_SERVER_PORT_,
				$int84$3608, $sym20$INTEGER,
				$str85$Possible_values__A_number___This_);
		register_system_parameter($sym86$_ALLOW_GUEST_TO_EDIT__, NIL,
				$sym16$T_OR_NIL, $str87$Possible_values__T__NIL___If_NIL_);
		register_system_parameter($sym88$_DEFAULT_CYCLIST_NAME_, $str89$Guest,
				$sym18$STRING, $str90$Possible_values__The_name_of_a_co);
		register_system_parameter($sym91$_HTML_IMAGE_DIRECTORY_,
				$str92$_cycdoc_img_, $sym18$STRING,
				$str93$The_directory_under_which_Cyc_ima);
		register_system_parameter($sym94$_HTML_JAVASCRIPT_DIRECTORY_,
				$str95$_cycdoc_js_, $sym18$STRING,
				$str96$The_directory_under_which_Javascr);
		register_system_parameter($sym97$_HTML_CSS_DIRECTORY_,
				$str98$_cycdoc_css_, $sym18$STRING,
				$str96$The_directory_under_which_Javascr);
		register_system_parameter($sym99$_CYC_EXECUTION_CONTEXT_,
				$kw100$UNKNOWN, $sym21$SYMBOL,
	...
	public static final SubLList $list0 = list(makeSymbol("T-OR-NIL"),
			makeSymbol("NIL-OR-STRING"), makeSymbol("STRING"),
			makeSymbol("FULL-PATH"), makeSymbol("INTEGER"),
			makeSymbol("SYMBOL"), makeSymbol("NONE"));
	...
	public static final SubLList $list7 = list(makeSymbol("NAME"),
			makeSymbol("DEFAULT"), makeSymbol("TYPE"),
			makeSymbol("DESCRIPTION"));
	...
	public static final SubLList $list13 = list(makeSymbol("SYMBOL"),
			makeSymbol("DEFAULT"), makeSymbol("TYPE"),
			makeSymbol("DESCRIPTION"));
	...
	public static final SubLObject $ic24 = Vectors.vector(new SubLObject[] {
			Characters.CHAR_0, Characters.CHAR_1, Characters.CHAR_2,
			Characters.CHAR_3, Characters.CHAR_4, Characters.CHAR_5,
			Characters.CHAR_6, Characters.CHAR_7, Characters.CHAR_8,
			Characters.CHAR_9, Characters.CHAR_A, Characters.CHAR_B,
			Characters.CHAR_C, Characters.CHAR_D, Characters.CHAR_E,
			Characters.CHAR_F });
	public static final SubLSymbol $kw25$LISP = makeKeyword("LISP");
	public static final SubLList $list26 = list(makeString("src"
	...
	public static final SubLList $list47 = list(makeSymbol("OPERATOR"),
			makeSymbol("&REST"), makeSymbol("ARGS"));
	public static final SubLSymbol $sym48$IN_PACKAGE = makeSymbol("IN-PACKAGE");
	public static final SubLList $list49 = list(makeSymbol("IN-PACKAGE"),
			makeString("CYC"));
	public static final SubLSymbol $sym50$CSETQ = makeSymbol("CSETQ");
	public static final SubLList $list51 = list(makeSymbol("SYMBOL"),
			makeSymbol("VALUE"));
