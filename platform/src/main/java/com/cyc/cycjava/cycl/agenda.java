...
public final class agenda extends SubLTranslatedFile {
	...
	public static final SubLObject initialize_agenda() {
		{
			...
			operation_communication
					.set_communication_mode(system_parameters.$startup_communication_mode$
							.getDynamicValue(thread));
			if (((NIL != system_parameters.$start_agenda_at_startupP$
					.getDynamicValue(thread)) && (NIL == agenda_running()))) {
	...
	@SubL(source = "cycl/agenda.lisp", position = 2893)
	 ...
	 */
	@SubL(source = "cycl/agenda.lisp", position = 4057)
	public static final SubLObject wait_for_agenda_not_running(
		...
		}
		{
			...
			for (; (!(((NIL == agenda_running()) || (NIL != Time
					.time_has_arrivedP(time)))));) {
	...
	public static final SubLObject agenda_top_level() {
		{
			...
			if ((NIL != current_process_is_agenda())) {
				Threads.set_process_priority(agenda_process(),
						Threads.$process_background_priority$.getGlobalValue());
				try {
					agenda_startup_actions();
					$restart_agenda_flag$.setGlobalValue(NIL);
					{
						SubLObject _prev_bind_0 = Packages.$package$
								.currentBinding(thread);
						try {
							Packages.$package$.bind(
									$default_agenda_package$.getGlobalValue(),
									thread);
							{
								SubLObject already_resourcing_p = sbhl_marking_vars.$resourcing_sbhl_marking_spaces_p$
										.getDynamicValue(thread);
								{
									SubLObject _prev_bind_0_1 = sbhl_marking_vars.$resourced_sbhl_marking_space_limit$
											.currentBinding(thread);
										...
										sbhl_marking_vars.$resourced_sbhl_marking_space_limit$
												.bind(sbhl_marking_vars
														.determine_resource_limit(
																...
																$agenda_resourcing_spaces$
																		.getDynamicValue(thread)),
														thread);
										sbhl_marking_vars.$resourced_sbhl_marking_spaces$
												.bind(sbhl_marking_vars
														.possibly_new_marking_resource(already_resourcing_p),
														thread);
										sbhl_marking_vars.$resourcing_sbhl_marking_spaces_p$
												.bind(T, thread);
										while ((NIL == $agenda_should_quit$
												.getGlobalValue())) {
											while (((NIL == $agenda_should_quit$
													.getGlobalValue()) && (NIL != agenda_work_to_do()))) {
												perform_one_agenda_action();
											}
											Threads.process_wait(
													$str3$Idle,
													Symbols.symbol_function($sym4$AGENDA_WORK_TO_DO));
										}
									} finally {
										sbhl_marking_vars.$resourcing_sbhl_marking_spaces_p$
	...
	@SubL(source = "cycl/agenda.lisp", position = 5828)
	...
	public static final SubLObject start_agenda(SubLObject waitP) {
		...
		Threads.make_process($str5$Cyc_Agenda,
				Symbols.symbol_function($sym6$AGENDA_TOP_LEVEL));
		if ((NIL != waitP)) {
	...
	public static final SubLObject agenda_work_to_do() {
		{
			...
			for (agenda_task = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
					.rest(), agenda_task = cdolist_list_var.first()) {
				{
					SubLObject test = agenda_task_test(agenda_task);
					if (((NIL != Symbols.fboundp(test)) && (NIL != Functions
							.funcall(test)))) {
						{
	...
	public static final class $agenda_work_to_do$ZeroArityFunction extends
			ZeroArityFunction {
				...
				if ((NIL != agenda_task)) {
					{
						SubLObject action = agenda_task_action(agenda_task);
						{
							SubLObject _prev_bind_0 = $within_agenda$
									.currentBinding(thread);
							try {
	...
	public static final SubLObject declare_agenda_task(SubLObject test,
			SubLObject action, SubLObject priority) {
		...
		undeclare_agenda_task(test);
		{
			...
			try {
				release = Locks.seize_lock(lock);
				$agenda_action_table$
						.setGlobalValue(list_utilities.splice_into_sorted_list(
								list(test, action, priority),
								$agenda_action_table$.getGlobalValue(),
								Symbols.symbol_function($sym19$_),
								Symbols.symbol_function($sym20$AGENDA_TASK_PRIORITY)));
			} finally {
				if ((NIL != release)) {
					Locks.release_lock(lock);
				}
	...
	public static final SubLObject undeclare_agenda_task(SubLObject test) {
		{
			...
			try {
				release = Locks.seize_lock(lock);
				$agenda_action_table$.setGlobalValue(Sequences.delete(test,
						$agenda_action_table$.getGlobalValue(),
						Symbols.symbol_function(EQL),
						Symbols.symbol_function($sym21$FIRST), UNPROVIDED,
	...
	public static final class $agenda_should_quitP$ZeroArityFunction extends
			ZeroArityFunction {
	...
	public static final class $restart_agenda_flagP$ZeroArityFunction extends
			ZeroArityFunction {
	...
	public static final SubLObject save_operationsP() {
		return makeBoolean(((NIL != operation_communication
				.saving_operationsP()) && (((NIL != Time
				.time_has_arrivedP($next_save_transcript_time$.getGlobalValue()))
				&& (NIL == operation_queues.transcript_queue_empty()) && (NIL != operation_queues
				.local_queue_empty())) || operation_queues
				.transcript_queue_size().numG(
						$transcript_queue_worry_size$.getGlobalValue()))));
	}

	public static final class $save_operationsP$ZeroArityFunction extends
			ZeroArityFunction {
	...
	public static final SubLObject save_local_operations() {
		$next_save_transcript_time$.setGlobalValue(numeric_date_utilities
				.time_from_now(Numbers.add(
						$save_transcript_quantum$.getGlobalValue(),
						random.random(TWENTY_INTEGER))));
	...
	public static final SubLObject worry_transmit_operationsP() {
		return makeBoolean(((NIL != operation_communication
				.allow_transmitting()) && (((NIL != Time
				.time_has_arrivedP($next_worry_transmit_time$.getGlobalValue())) && (NIL == operation_queues
				.transmit_queue_empty())) || operation_queues
				.transmit_queue_size().numGE(
						$worry_transmit_size$.getGlobalValue()))));
	}

	public static final class $worry_transmit_operationsP$ZeroArityFunction
			extends ZeroArityFunction {
	...
	public static final SubLObject run_auxiliary_opP() {
		return makeBoolean(((NIL != operation_communication
				.process_auxiliary_operationsP()) && (NIL == operation_queues
				.auxiliary_queue_empty())));
	}

	public static final class $run_auxiliary_opP$ZeroArityFunction extends
			ZeroArityFunction {
	...
	public static final SubLObject run_local_opP() {
		return makeBoolean(((NIL != operation_communication
				.process_local_operationsP()) && (NIL == operation_queues
				.local_queue_empty())));
	}

	public static final class $run_local_opP$ZeroArityFunction extends
			ZeroArityFunction {
	...
	public static final SubLObject normal_transmit_operationsP() {
		return makeBoolean(((NIL != Time
				.time_has_arrivedP($next_normal_transmit_time$.getGlobalValue()))
				&& (NIL == operation_queues.transmit_queue_empty())
				&& (NIL != operation_communication.allow_transmitting())
				&& (NIL != operation_queues.local_queue_empty()) && (NIL != operation_queues
	...
	public static final class $run_remote_opP$ZeroArityFunction extends
			ZeroArityFunction {
	...
	public static final SubLObject load_operationsP() {
		return makeBoolean(((NIL != operation_communication
				.receiving_remote_operationsP()) && (NIL != Time
				.time_has_arrivedP($next_load_transcript_time$.getGlobalValue()))));
	}

	public static final class $load_operationsP$ZeroArityFunction extends
			ZeroArityFunction {
	...
	public static final SubLObject save_experienceP() {
		return makeBoolean(((NIL != $save_recent_experienceP$.getGlobalValue()) && (NIL != Time
				.time_has_arrivedP($next_save_experience_transcript_time$
						.getGlobalValue()))));
	}

	public static final class $save_experienceP$ZeroArityFunction extends
			ZeroArityFunction {
	...
	public static final SubLObject save_local_experience() {
		$next_save_experience_transcript_time$
				.setGlobalValue(numeric_date_utilities
						.time_from_now($save_experience_transcript_quantum$
								.getGlobalValue()));
	...
	public static final SubLObject agenda_save_asked_queriesP() {
		return makeBoolean(((NIL != control_vars.save_asked_queriesP()) && (NIL != Time
				.time_has_arrivedP($next_save_asked_queries_transcript_time$
						.getGlobalValue()))));
	}
	...
	public static final SubLObject agenda_daily_gc_ready_p() {
		return makeBoolean(((NIL != $agenda_daily_gc_enabled$.getGlobalValue())
				&& (NIL != $next_agenda_daily_gc_time$.getGlobalValue()) && (NIL != Time
				.time_has_arrivedP($next_agenda_daily_gc_time$.getGlobalValue()))));
	}

	public static final class $agenda_daily_gc_ready_p$ZeroArityFunction extends
			ZeroArityFunction {
	...
	public static final SubLObject declare_agenda_file() {
		declareFunction(myName, "initialize_agenda", "INITIALIZE-AGENDA", 0, 0,
				false);
		declareFunction(myName, "within_agenda", "WITHIN-AGENDA", 0, 0, false);
		declareFunction(myName, "agenda_form_to_show", "AGENDA-FORM-TO-SHOW",
				0, 0, false);
		declareFunction(myName, "agenda_top_level", "AGENDA-TOP-LEVEL", 0, 0,
				false);
		...
		declareFunction(myName, "abort_and_restart_agenda",
				"ABORT-AND-RESTART-AGENDA", 0, 0, false);
		declareFunction(myName, "wait_for_agenda_running",
				"WAIT-FOR-AGENDA-RUNNING", 0, 1, false);
		declareFunction(myName, "wait_for_agenda_not_running",
				"WAIT-FOR-AGENDA-NOT-RUNNING", 0, 1, false);
		declareFunction(myName, "ensure_agenda_running",
				"ENSURE-AGENDA-RUNNING", 0, 0, false);
		...
		declareFunction(myName, "current_process_is_agenda",
				"CURRENT-PROCESS-IS-AGENDA", 0, 0, false);
		declareFunction(myName, "agenda_idleP", "AGENDA-IDLE?", 0, 0, false);
		declareFunction(myName, "agenda_busyP", "AGENDA-BUSY?", 0, 0, false);
		declareFunction(myName, "start_agenda_process", "START-AGENDA-PROCESS",
				0, 0, false);
		declareFunction(myName, "clear_agenda_process", "CLEAR-AGENDA-PROCESS",
				0, 0, false);
		declareFunction(myName, "agenda_error_modes", "AGENDA-ERROR-MODES", 0,
				0, false);
		declareFunction(myName, "agenda_error_mode", "AGENDA-ERROR-MODE", 0, 0,
				false);
		declareFunction(myName, "set_agenda_error_mode",
				"SET-AGENDA-ERROR-MODE", 1, 0, false);
		declareFunction(myName, "agenda_logs_errorsP", "AGENDA-LOGS-ERRORS?",
				0, 0, false);
		declareFunction(myName, "set_agenda_log_file", "SET-AGENDA-LOG-FILE",
				1, 0, false);
		declareFunction(myName, "get_agenda_log_file", "GET-AGENDA-LOG-FILE",
				0, 0, false);
		declareFunction(myName, "agenda_startup_actions",
				"AGENDA-STARTUP-ACTIONS", 0, 0, false);
		declareFunction(myName, "agenda_work_to_do", "AGENDA-WORK-TO-DO", 0, 0,
				false);
		new $agenda_work_to_do$ZeroArityFunction();
		declareFunction(myName, "clear_agenda_halt_explanation",
				"CLEAR-AGENDA-HALT-EXPLANATION", 0, 0, false);
		declareFunction(myName, "perform_one_agenda_action",
				"PERFORM-ONE-AGENDA-ACTION", 0, 0, false);
		declareFunction(myName, "declare_agenda_task", "DECLARE-AGENDA-TASK",
				3, 0, false);
		declareFunction(myName, "undeclare_agenda_task",
				"UNDECLARE-AGENDA-TASK", 1, 0, false);
		declareFunction(myName, "agenda_tasks", "AGENDA-TASKS", 0, 0, false);
		declareFunction(myName, "agenda_task_test", "AGENDA-TASK-TEST", 1, 0,
				false);
		declareFunction(myName, "agenda_task_action", "AGENDA-TASK-ACTION", 1,
				0, false);
		declareFunction(myName, "agenda_task_priority", "AGENDA-TASK-PRIORITY",
				1, 0, false);
		declareFunction(myName, "agenda_should_quitP", "AGENDA-SHOULD-QUIT?",
				0, 0, false);
		new $agenda_should_quitP$ZeroArityFunction();
		declareFunction(myName, "do_nothing", "DO-NOTHING", 0, 0, false);
		declareFunction(myName, "restart_agenda_flagP", "RESTART-AGENDA-FLAG?",
				0, 0, false);
		new $restart_agenda_flagP$ZeroArityFunction();
		declareFunction(myName, "save_operationsP", "SAVE-OPERATIONS?", 0, 0,
				false);
		new $save_operationsP$ZeroArityFunction();
		declareFunction(myName, "save_local_operations",
				"SAVE-LOCAL-OPERATIONS", 0, 0, false);
		declareFunction(myName, "worry_transmit_operationsP",
				"WORRY-TRANSMIT-OPERATIONS?", 0, 0, false);
		new $worry_transmit_operationsP$ZeroArityFunction();
		declareFunction(myName, "worry_transmit_operations",
				"WORRY-TRANSMIT-OPERATIONS", 0, 0, false);
		declareFunction(myName, "run_auxiliary_opP", "RUN-AUXILIARY-OP?", 0, 0,
				false);
		new $run_auxiliary_opP$ZeroArityFunction();
		declareFunction(myName, "run_one_auxiliary_op_in_agenda",
				"RUN-ONE-AUXILIARY-OP-IN-AGENDA", 0, 0, false);
		declareFunction(myName, "run_one_non_local_op_in_agenda",
				"RUN-ONE-NON-LOCAL-OP-IN-AGENDA", 1, 0, false);
		declareFunction(myName, "handle_agenda_fi_error_state",
				"HANDLE-AGENDA-FI-ERROR-STATE", 0, 0, false);
		declareFunction(myName, "run_local_opP", "RUN-LOCAL-OP?", 0, 0, false);
		new $run_local_opP$ZeroArityFunction();
		declareFunction(myName, "run_one_local_op_in_agenda",
				"RUN-ONE-LOCAL-OP-IN-AGENDA", 0, 0, false);
		declareFunction(myName, "agenda_throw_error_message",
				"AGENDA-THROW-ERROR-MESSAGE", 0, 0, false);
		declareFunction(myName, "log_agenda_error", "LOG-AGENDA-ERROR", 1, 0,
				false);
		declareFunction(myName, "timestamp_operation_p",
				"TIMESTAMP-OPERATION-P", 1, 0, false);
		declareFunction(myName, "construct_generic_timestamp_operation",
				"CONSTRUCT-GENERIC-TIMESTAMP-OPERATION", 0, 0, false);
		declareFunction(myName, "normal_transmit_operationsP",
				"NORMAL-TRANSMIT-OPERATIONS?", 0, 0, false);
		new $normal_transmit_operationsP$ZeroArityFunction();
		declareFunction(myName, "normal_transmit_operations",
				"NORMAL-TRANSMIT-OPERATIONS", 0, 0, false);
		declareFunction(myName, "run_remote_opP", "RUN-REMOTE-OP?", 0, 0, false);
		new $run_remote_opP$ZeroArityFunction();
		declareFunction(myName, "run_one_remote_op_in_agenda",
				"RUN-ONE-REMOTE-OP-IN-AGENDA", 0, 0, false);
		declareFunction(myName, "load_operationsP", "LOAD-OPERATIONS?", 0, 0,
				false);
		new $load_operationsP$ZeroArityFunction();
		declareFunction(myName, "load_remote_operations",
				"LOAD-REMOTE-OPERATIONS", 0, 0, false);
		declareFunction(myName, "save_experienceP", "SAVE-EXPERIENCE?", 0, 0,
				false);
		new $save_experienceP$ZeroArityFunction();
		declareFunction(myName, "save_local_experience",
				"SAVE-LOCAL-EXPERIENCE", 0, 0, false);
		declareFunction(myName, "agenda_save_asked_queriesP",
				"AGENDA-SAVE-ASKED-QUERIES?", 0, 0, false);
		new $agenda_save_asked_queriesP$ZeroArityFunction();
		declareFunction(myName, "save_local_asked_queries",
				"SAVE-LOCAL-ASKED-QUERIES", 0, 0, false);
		declareFunction(myName, "enable_agenda_daily_gc",
				"ENABLE-AGENDA-DAILY-GC", 3, 0, false);
		declareFunction(myName, "disable_agenda_daily_gc",
				"DISABLE-AGENDA-DAILY-GC", 0, 0, false);
		declareFunction(myName, "agenda_daily_gc_ready_p",
				"AGENDA-DAILY-GC-READY-P", 0, 0, false);
		new $agenda_daily_gc_ready_p$ZeroArityFunction();
		declareFunction(myName, "do_agenda_daily_gc", "DO-AGENDA-DAILY-GC", 0,
				0, false);
		declareFunction(myName, "set_next_agenda_daily_gc_time",
				"SET-NEXT-AGENDA-DAILY-GC-TIME", 0, 0, false);
		declareFunction(myName, "update_agenda_daily_gc_time",
				"UPDATE-AGENDA-DAILY-GC-TIME", 0, 0, false);
		return NIL;
	}

	public static final SubLObject init_agenda_file() {
		$agenda_should_quit$ = deflexical(
				"*AGENDA-SHOULD-QUIT*",
				((NIL != Symbols.boundp($sym0$_AGENDA_SHOULD_QUIT_)) ? ((SubLObject) $agenda_should_quit$
						.getGlobalValue()) : NIL));
		$restart_agenda_flag$ = deflexical(
				"*RESTART-AGENDA-FLAG*",
				((NIL != Symbols.boundp($sym1$_RESTART_AGENDA_FLAG_)) ? ((SubLObject) $restart_agenda_flag$
						.getGlobalValue()) : NIL));
		$within_agenda$ = defparameter("*WITHIN-AGENDA*", NIL);
		$default_agenda_package$ = deflexical("*DEFAULT-AGENDA-PACKAGE*",
				Packages.find_package($str2$CYC));
		$agenda_resourcing_spaces$ = defparameter("*AGENDA-RESOURCING-SPACES*",
				TEN_INTEGER);
		$agenda_process$ = deflexical(
				"*AGENDA-PROCESS*",
				((NIL != Symbols.boundp($sym7$_AGENDA_PROCESS_)) ? ((SubLObject) $agenda_process$
						.getGlobalValue()) : NIL));
		$agenda_process_lock$ = deflexical("*AGENDA-PROCESS-LOCK*",
				Locks.make_lock($str10$Agenda_Process_Lock));
		$agenda_error_modes$ = deflexical("*AGENDA-ERROR-MODES*", $list11);
		$agenda_error_mode$ = deflexical(
				"*AGENDA-ERROR-MODE*",
				((NIL != Symbols.boundp($sym12$_AGENDA_ERROR_MODE_)) ? ((SubLObject) $agenda_error_mode$
						.getGlobalValue()) : $kw13$HALT));
		$agenda_log_file$ = defvar("*AGENDA-LOG-FILE*", NIL);
		$agenda_action_table$ = deflexical(
				"*AGENDA-ACTION-TABLE*",
				((NIL != Symbols.boundp($sym15$_AGENDA_ACTION_TABLE_)) ? ((SubLObject) $agenda_action_table$
						.getGlobalValue()) : NIL));
		$agenda_action_table_lock$ = deflexical("*AGENDA-ACTION-TABLE-LOCK*",
				Locks.make_lock($str16$Agenda_Action_Table_Lock));
		$transcript_queue_worry_size$ = deflexical(
				"*TRANSCRIPT-QUEUE-WORRY-SIZE*", TWENTY_INTEGER);
		$save_transcript_quantum$ = deflexical("*SAVE-TRANSCRIPT-QUANTUM*",
				$int25$60);
		$next_save_transcript_time$ = deflexical(
				"*NEXT-SAVE-TRANSCRIPT-TIME*",
				((NIL != Symbols.boundp($sym26$_NEXT_SAVE_TRANSCRIPT_TIME_)) ? ((SubLObject) $next_save_transcript_time$
						.getGlobalValue()) : Time.get_universal_time()));
		$worry_transmit_quantum$ = deflexical("*WORRY-TRANSMIT-QUANTUM*",
				Numbers.multiply(TEN_INTEGER, $int25$60));
		$worry_transmit_size$ = deflexical("*WORRY-TRANSMIT-SIZE*", $int29$1000);
		$next_worry_transmit_time$ = deflexical(
				"*NEXT-WORRY-TRANSMIT-TIME*",
				((NIL != Symbols.boundp($sym30$_NEXT_WORRY_TRANSMIT_TIME_)) ? ((SubLObject) $next_worry_transmit_time$
						.getGlobalValue()) : Time.get_universal_time()));
		$normal_transmit_quantum$ = deflexical("*NORMAL-TRANSMIT-QUANTUM*",
				$int59$120);
		$next_normal_transmit_time$ = deflexical(
				"*NEXT-NORMAL-TRANSMIT-TIME*",
				((NIL != Symbols.boundp($sym60$_NEXT_NORMAL_TRANSMIT_TIME_)) ? ((SubLObject) $next_normal_transmit_time$
						.getGlobalValue()) : Time.get_universal_time()));
		$load_transcript_quantum$ = deflexical("*LOAD-TRANSCRIPT-QUANTUM*",
				$int59$120);
		$next_load_transcript_time$ = deflexical(
				"*NEXT-LOAD-TRANSCRIPT-TIME*",
				((NIL != Symbols.boundp($sym68$_NEXT_LOAD_TRANSCRIPT_TIME_)) ? ((SubLObject) $next_load_transcript_time$
						.getGlobalValue()) : Time.get_universal_time()));
		$save_recent_experienceP$ = deflexical("*SAVE-RECENT-EXPERIENCE?*", T);
		$save_experience_transcript_quantum$ = deflexical(
				"*SAVE-EXPERIENCE-TRANSCRIPT-QUANTUM*",
				Numbers.multiply($int25$60, TEN_INTEGER));
		$next_save_experience_transcript_time$ = deflexical(
				"*NEXT-SAVE-EXPERIENCE-TRANSCRIPT-TIME*",
				((NIL != Symbols
						.boundp($sym72$_NEXT_SAVE_EXPERIENCE_TRANSCRIPT_TIME_)) ? ((SubLObject) $next_save_experience_transcript_time$
						.getGlobalValue()) : Time.get_universal_time()));
		$save_asked_queries_transcript_quantum$ = deflexical(
				"*SAVE-ASKED-QUERIES-TRANSCRIPT-QUANTUM*",
				Numbers.multiply($int25$60, ONE_INTEGER));
		$next_save_asked_queries_transcript_time$ = deflexical(
				"*NEXT-SAVE-ASKED-QUERIES-TRANSCRIPT-TIME*",
				((NIL != Symbols
						.boundp($sym75$_NEXT_SAVE_ASKED_QUERIES_TRANSCRIPT_TIME_)) ? ((SubLObject) $next_save_asked_queries_transcript_time$
						.getGlobalValue()) : Time.get_universal_time()));
		$agenda_daily_gc_lock$ = deflexical("*AGENDA-DAILY-GC-LOCK*",
				Locks.make_lock($str79$Agenda_Daily_GC));
		$agenda_daily_gc_enabled$ = deflexical(
				"*AGENDA-DAILY-GC-ENABLED*",
				((NIL != Symbols.boundp($sym80$_AGENDA_DAILY_GC_ENABLED_)) ? ((SubLObject) $agenda_daily_gc_enabled$
						.getGlobalValue()) : NIL));
		$agenda_daily_gc_time_of_day$ = deflexical(
				"*AGENDA-DAILY-GC-TIME-OF-DAY*",
				((NIL != Symbols.boundp($sym81$_AGENDA_DAILY_GC_TIME_OF_DAY_)) ? ((SubLObject) $agenda_daily_gc_time_of_day$
						.getGlobalValue()) : $list82));
		$next_agenda_daily_gc_time$ = deflexical(
				"*NEXT-AGENDA-DAILY-GC-TIME*",
				((NIL != Symbols.boundp($sym88$_NEXT_AGENDA_DAILY_GC_TIME_)) ? ((SubLObject) $next_agenda_daily_gc_time$
						.getGlobalValue()) : NIL));
		return NIL;
	}
	...
	public static final SubLObject setup_agenda_file() {
		...
		declare_agenda_task($sym22$AGENDA_SHOULD_QUIT_, $sym23$DO_NOTHING,
				ZERO_INTEGER);
		declare_agenda_task($sym24$RESTART_AGENDA_FLAG_, $sym23$DO_NOTHING,
				ZERO_INTEGER);
		subl_macro_promotions
				.declare_defglobal($sym26$_NEXT_SAVE_TRANSCRIPT_TIME_);
		declare_agenda_task($sym27$SAVE_OPERATIONS_,
				$sym28$SAVE_LOCAL_OPERATIONS, TEN_INTEGER);
		subl_macro_promotions
				.declare_defglobal($sym30$_NEXT_WORRY_TRANSMIT_TIME_);
		declare_agenda_task($sym31$WORRY_TRANSMIT_OPERATIONS_,
				$sym32$WORRY_TRANSMIT_OPERATIONS, FIFTEEN_INTEGER);
		declare_agenda_task($sym34$RUN_AUXILIARY_OP_,
				$sym35$RUN_ONE_AUXILIARY_OP_IN_AGENDA, TWENTY_INTEGER);
		declare_agenda_task($sym50$RUN_LOCAL_OP_,
				$sym51$RUN_ONE_LOCAL_OP_IN_AGENDA, $int52$25);
		subl_macro_promotions
				.declare_defglobal($sym60$_NEXT_NORMAL_TRANSMIT_TIME_);
		declare_agenda_task($sym61$NORMAL_TRANSMIT_OPERATIONS_,
				$sym62$NORMAL_TRANSMIT_OPERATIONS, $int63$35);
		declare_agenda_task($sym65$RUN_REMOTE_OP_,
				$sym66$RUN_ONE_REMOTE_OP_IN_AGENDA, $int67$40);
		subl_macro_promotions
				.declare_defglobal($sym68$_NEXT_LOAD_TRANSCRIPT_TIME_);
		declare_agenda_task($sym69$LOAD_OPERATIONS_,
				$sym70$LOAD_REMOTE_OPERATIONS, $int71$45);
		subl_macro_promotions
				.declare_defglobal($sym72$_NEXT_SAVE_EXPERIENCE_TRANSCRIPT_TIME_);
		declare_agenda_task($sym73$SAVE_EXPERIENCE_,
				$sym74$SAVE_LOCAL_EXPERIENCE, $int25$60);
		subl_macro_promotions
				.declare_defglobal($sym75$_NEXT_SAVE_ASKED_QUERIES_TRANSCRIPT_TIME_);
		declare_agenda_task($sym76$AGENDA_SAVE_ASKED_QUERIES_,
				$sym77$SAVE_LOCAL_ASKED_QUERIES, $int78$30);
		subl_macro_promotions
				.declare_defglobal($sym80$_AGENDA_DAILY_GC_ENABLED_);
		subl_macro_promotions
				.declare_defglobal($sym81$_AGENDA_DAILY_GC_TIME_OF_DAY_);
		subl_macro_promotions
				.declare_defglobal($sym88$_NEXT_AGENDA_DAILY_GC_TIME_);
		declare_agenda_task($sym90$AGENDA_DAILY_GC_READY_P,
				$sym91$DO_AGENDA_DAILY_GC, $int29$1000);
		return NIL;
	}
	...
	public static final SubLList $list11 = list(makeKeyword("IGNORE"),
			makeKeyword("HALT"), makeKeyword("DEBUG"), makeKeyword("LOG"));
	...
	public static final SubLList $list38 = list(makeSymbol("ERROR"),
			makeSymbol("&OPTIONAL"), makeSymbol("FORMAT-STRING"),
			makeSymbol("&REST"), makeSymbol("ARGS"));
	...
	public static final SubLList $list43 = list(makeSymbol("WARNING"),
			makeSymbol("&OPTIONAL"), makeSymbol("FORMAT-STRING"),
			makeSymbol("&REST"), makeSymbol("ARGS"));
	...
	public static final SubLObject $const57$Guest = constant_handles
	...
	public static final SubLList $list82 = list(FOUR_INTEGER, ZERO_INTEGER,
			ZERO_INTEGER);
	...
	public static final SubLList $list92 = list(makeSymbol("HOUR"),
			makeSymbol("MINUTE"), makeSymbol("SECOND"));
