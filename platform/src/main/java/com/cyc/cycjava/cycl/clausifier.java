...
import com.cyc.cycjava.cycl.czer_trampolines;
...
public  final class clausifier extends SubLTranslatedFile {
  ...
  @SubL(source = "cycl/clausifier.lisp", position = 26052) 
  ...
  public static final SubLObject recursively_standardize_variables(SubLObject sentence) {
    {
      ...
      if ((NIL == el_utilities.el_formula_p(sentence))) {
        ...
        {
          SubLObject quantifier = el_utilities.unmake_binary_formula(sentence);
          ...
          thread.resetMultipleValues();
          {
            ...
            SubLObject new_var = el_uniquify_standardize(old_var);
            SubLObject replace_old_var = el_var_without_quote(old_var);
            SubLObject replace_new_var = el_var_without_quote(new_var);
            SubLObject standardized = NIL;
            remember_variable_rename(replace_old_var,replace_new_var);
            {
              ...
              try {
                ...
                standardized = el_utilities.make_binary_formula(quantifier, new_var, cycl_utilities.expression_nsubst_free_vars(replace_new_var,replace_old_var,standardized_subform,UNPROVIDED));
              } finally {
  ...
  @SubL(source = "cycl/clausifier.lisp", position = 25878) 
  public static final SubLObject el_var_without_quote(SubLObject var) {
    if (((NIL != cycl_grammar.fast_escape_quote_term_p(var))
        || (NIL != cycl_grammar.fast_quasi_quote_term_p(var)))) {
  ...
  @SubL(source = "cycl/clausifier.lisp", position = 25589) 
  public static final SubLObject el_uniquify_standardize(SubLObject var) {
    {
      ...
      } else if (((NIL != cycl_grammar.fast_escape_quote_term_p(var))
          || (NIL != cycl_grammar.fast_quasi_quote_term_p(var)))) {
  ...
  @SubL(source = "cycl/clausifier.lisp", position = 27177) 
  public static final SubLObject el_uniquify(SubLObject var) {
    {
      ...
      thread.resetMultipleValues();
      {
        ...
        }
        {
          ...
          {
            SubLObject unique_string = (n.isZero() ? ((SubLObject) format_nil.format_nil_a(cycl_variables.variable_name(symbol))) : Sequences.cconcatenate(format_nil.format_nil_a_no_copy(cycl_variables.variable_name(symbol)), new SubLObject[] {$str23$_, format_nil.format_nil_a_no_copy(string_utilities.object_to_string(n))}));
            if ((NIL != cycl_variables.keyword_varP(symbol))) {
              return Errors
				.handleMissingMethodError("This call was replaced for LarKC purposes. Removed this method:");//cycl_variables.make_keyword_var(unique_string);
            } else {
  ...
  @SubL(source = "cycl/clausifier.lisp", position = 28848) 
          ...
          try {
            ...
            if ((NIL != el_utilities.cycl_universal_p(sentence))) {
              thread.resetMultipleValues();
              {
                ...
                SubLObject subform = thread.thirdMultipleValue();
                thread.resetMultipleValues();
                $quantifier_info_list$.setDynamicValue(cons(list(quantifier, NIL, var, el_utilities.sentence_free_term_variables(subform, UNPROVIDED, UNPROVIDED), el_utilities.sentence_free_sequence_variables(subform, UNPROVIDED, UNPROVIDED)), $quantifier_info_list$.getDynamicValue(thread)), thread);
              ...
              thread.resetMultipleValues();
              {
                SubLObject quantifier = el_utilities.unmake_ternary_formula(sentence);
                SubLObject num = thread.secondMultipleValue();
                SubLObject var = thread.thirdMultipleValue();
                SubLObject subform = thread.fourthMultipleValue();
                thread.resetMultipleValues();
                $quantifier_info_list$.setDynamicValue(cons(list(quantifier, num, var, el_utilities.sentence_free_term_variables(subform, UNPROVIDED, UNPROVIDED), el_utilities.sentence_free_sequence_variables(subform, UNPROVIDED, UNPROVIDED)), $quantifier_info_list$.getDynamicValue(thread)), thread);
            ...
            } else if (((NIL != control_vars.within_askP())
                 && (NIL != el_utilities.ist_sentence_p(sentence)))) {
              thread.resetMultipleValues();
              {
                SubLObject ist = el_utilities.unmake_binary_formula(sentence);
                SubLObject mt = thread.secondMultipleValue();
                SubLObject subsentence = thread.thirdMultipleValue();
                thread.resetMultipleValues();
                {
                  SubLObject canonical_subsentence = existentials_out_int(subsentence);
  ...
  public static final SubLObject skolemize_atomic_sentence(SubLObject sentence, SubLObject quantifier_info_list) {
    {
      ...
      for (quantifier_info_list_in_scope = quantifier_info_list, curr_quant_info = quantifier_info_list_in_scope.first(), curr_quant = curr_quant_info.first(), curr_num = conses_high.second(curr_quant_info), curr_var = conses_high.third(curr_quant_info), curr_free_term_vars = conses_high.fourth(curr_quant_info), curr_free_sequence_vars = conses_high.fifth(curr_quant_info); (NIL != quantifier_info_list_in_scope); quantifier_info_list_in_scope = quantifier_info_list_in_scope.rest(), curr_quant_info = quantifier_info_list_in_scope.first(), curr_quant = curr_quant_info.first(), curr_num = conses_high.second(curr_quant_info), curr_var = conses_high.third(curr_quant_info), curr_free_term_vars = conses_high.fourth(curr_quant_info), curr_free_sequence_vars = conses_high.fifth(curr_quant_info)) {
...
existentials, simply drop them (like it does by default during asks)? */
  @SubL(source = "cycl/clausifier.lisp", position = 40132) 
  public static final SubLObject drop_all_existentialsP() {
    {
      final SubLThread thread = SubLProcess.currentSubLThread();
      return makeBoolean((((NIL != control_vars.$within_ask$.getDynamicValue(thread))
               && (NIL == czer_vars.$skolemize_during_asksP$.getDynamicValue(thread)))
            || (NIL != czer_vars.$drop_all_existentialsP$.getDynamicValue(thread))));
    }
  }


  @SubL(source = "cycl/clausifier.lisp", position = 41158) 
  public static final SubLObject forbid_quantified_sequence_variablesP() {
    {
      ...
      if ((T == czer_vars.$forbid_quantified_sequence_variablesP$.getDynamicValue(thread))) {
        return T;
      } else if ((NIL == czer_vars.$forbid_quantified_sequence_variablesP$.getDynamicValue(thread))) {
        return NIL;
      } else if (($kw41$ASSERT_ONLY == czer_vars.$forbid_quantified_sequence_variablesP$.getDynamicValue(thread))) {
        return control_vars.within_assertP();
      } else {
        return Errors.error($str42$Unexpected_value_for__forbid_quan, czer_vars.$forbid_quantified_sequence_variablesP$.getDynamicValue(thread));
      }
    }
  }
  ...
  public static final SubLObject skolemize_variable(SubLObject sentence, SubLObject curr_quant, SubLObject curr_num, SubLObject curr_var, SubLObject curr_free_term_vars, SubLObject curr_free_sequence_vars, SubLObject quantifier_info_list_in_scope) {
    if ((NIL != drop_all_existentialsP())) {
      return sentence;
    } else if (((NIL != el_utilities.occurs_as_sequence_variableP(curr_var, sentence))
         && (NIL != forbid_quantified_sequence_variablesP()))) {
      Dynamic.sublisp_throw($kw27$QUANTIFIED_SEQUENCE_VARIABLE, list($kw27$QUANTIFIED_SEQUENCE_VARIABLE, curr_var, sentence));
    } else {
      {
        ...
        for (quant_info = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var.rest(), quant_info = cdolist_list_var.first()) {
          {
            ...
            SubLObject var = conses_high.third(quant_info);
            SubLObject free_term_vars = conses_high.fourth(quant_info);
            SubLObject free_sequence_vars = conses_high.fifth(quant_info);
            if (($const30$forAll == quant)) {
              {
                SubLObject var_status = determine_skolem_var_status(var, curr_free_term_vars, free_term_vars, curr_free_sequence_vars, free_sequence_vars);
                SubLObject pcase_var = var_status;
                if (pcase_var.eql($kw31$NEITHER)) {
                } else if (pcase_var.eql($kw32$TERM)) {
                  ...
                  Dynamic.sublisp_throw($kw26$AMBIGUOUS_VAR_TYPE_IN_SKOLEM_SCOPE, list($kw26$AMBIGUOUS_VAR_TYPE_IN_SKOLEM_SCOPE, var, curr_var, sentence));
        ...
        if (Sequences.length(curr_dependent_sequence_vars).numG(ONE_INTEGER)) {
          Dynamic.sublisp_throw($kw25$TOO_MANY_SEQUENCE_VARS_IN_SKOLEM_SCOPE, list($kw25$TOO_MANY_SEQUENCE_VARS_IN_SKOLEM_SCOPE, curr_var, curr_dependent_sequence_vars));
        } else if (((NIL != leave_skolem_constants_aloneP())
             && (NIL == curr_dependent_term_vars)
             && (NIL == curr_dependent_sequence_vars))) {
          return sentence;
        } else {
          return cycl_utilities.expression_nsubst_free_vars(make_skolem_fn_fn(curr_var, curr_dependent_term_vars, curr_quant, curr_num, curr_dependent_sequence_vars.first()), curr_var, el_utilities.copy_expression(sentence), Symbols.symbol_function(EQUAL));
        }
      }
    }
    return NIL;
  ...
  @SubL(source = "cycl/clausifier.lisp", position = 41031) 
  public static final SubLObject leave_skolem_constants_aloneP() {
    {
      final SubLThread thread = SubLProcess.currentSubLThread();
      return makeBoolean(((NIL != drop_all_existentialsP())
            || (NIL != czer_vars.$leave_skolem_constants_aloneP$.getDynamicValue(thread))));
    }
  }

  /** @param DEPENDENT-SEQUENCE-VAR; nil or cycl-variable-p */
  @SubL(source = "cycl/clausifier.lisp", position = 38839) 
  public static final SubLObject make_skolem_fn_fn(SubLObject var, SubLObject dependent_term_vars, SubLObject quant, SubLObject num, SubLObject dependent_sequence_var) {
    {
      final SubLThread thread = SubLProcess.currentSubLThread();
      if (((NIL == dependent_term_vars)
           && (NIL == dependent_sequence_var)
           && (NIL != czer_vars.$use_skolem_constantsP$.getDynamicValue(thread)))) {
        Errors.warn($str37$skolem_constants_not_yet_supporte);
      } else {
        {
          SubLObject result = NIL;
          if (($const38$thereExists == quant)) {
            result = el_utilities.make_ternary_formula($const39$SkolemFunctionFn, dependent_term_vars, var, dependent_sequence_var);
          } else {
            el_utilities.el_error(FOUR_INTEGER, $str40$make_skolem_fn_fn_doesn_t_know_ho, quant, UNPROVIDED, UNPROVIDED);
            return NIL;
          }
          return result;
        }
  ...
  /** @param VAR; the universally scoped variable whose status we need to determine wrt an existential.
  ...
  @SubL(source = "cycl/clausifier.lisp", position = 37354) 
  public static final SubLObject determine_skolem_var_status(SubLObject var, SubLObject subsent_free_term_vars, SubLObject free_term_vars_in_scope, SubLObject subsent_free_seqvars, SubLObject free_seqvars_in_scope) {
    {
      final SubLThread thread = SubLProcess.currentSubLThread();
      {
        SubLObject var_status = $kw35$UNDETERMINED;
        if ((NIL != conses_high.member(var, subsent_free_term_vars, UNPROVIDED, UNPROVIDED))) {
          if (((NIL != subl_promotions.memberP(var, subsent_free_seqvars, UNPROVIDED, UNPROVIDED))
              || (NIL != conses_high.member(var, free_seqvars_in_scope, UNPROVIDED, UNPROVIDED)))) {
          ...
          if ((NIL != czer_vars.$minimal_skolem_arityP$.getDynamicValue(thread))) {
            ...
            if ((NIL != conses_high.member(var, free_term_vars_in_scope, UNPROVIDED, UNPROVIDED))) {
