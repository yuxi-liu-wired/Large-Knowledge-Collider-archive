...
public final class transcript_utilities extends SubLTranslatedFile {
	...
	/**
	 * Modifies *local-transcript* by adding the mark into the name. If the file
	 * exists on disk, then rename the file to the new name.
	 */
	@SubL(source = "cycl/transcript-utilities.lisp", position = 5289)
	public static final SubLObject mark_local_transcript(SubLObject mark) {
		{
			SubLObject oldname = $local_transcript$.getGlobalValue();
			if ((NIL != oldname)) {
				{
					SubLObject newname = mark_transcript_filename(oldname, mark);
	...
	@SubL(source = "cycl/transcript-utilities.lisp", position = 12926)
	public static final SubLObject mark_transcript_filename(
			SubLObject transcript, SubLObject mark) {
		checkType(transcript, $sym24$STRINGP);
		checkType(mark, $sym24$STRINGP);
		if ((NIL != string_utilities.substringP($str25$_TS, transcript,
				Symbols.symbol_function(EQUAL), UNPROVIDED, UNPROVIDED))) {
			return string_utilities.replace_substring(
					transcript,
					$str25$_TS,
					Sequences.cconcatenate(
							$str26$_,
							new SubLObject[] {
									Strings.string_upcase(mark, UNPROVIDED,
											UNPROVIDED), $str25$_TS }));
		} else {
			return string_utilities.replace_substring(
					transcript,
					$str27$_ts,
					Sequences.cconcatenate(
							$str26$_,
							new SubLObject[] {
									Strings.string_downcase(mark, UNPROVIDED,
											UNPROVIDED), $str27$_ts }));
		}
	}
	...
	/**
	 * Boolean: has the master transcript been accessed -- probed, touched,
	 * written to, or read from -- yet?
	 */
	...
	public static final SubLObject set_master_transcript_already_exists(
			SubLObject number) {
		{
			final SubLThread thread = SubLProcess.currentSubLThread();
	...
	/**
	 * Clear local-operation-storage-queue and set *local-transcript* to a new
	 * file.
	 */
	...
	public static final SubLObject new_local_transcript_int() {
		if (((NIL != $local_transcript$.getGlobalValue()) && (NIL != Filesys
				.probe_file($local_transcript$.getGlobalValue())))) {
			Errors.handleMissingMethodError("This call was replaced for LarKC purposes. Originally a method was called. Refer to number 6062");
			Errors.handleMissingMethodError("This call was replaced for LarKC purposes. Originally a method was called. Refer to number 6055");
		}
		$local_transcript$
				.setGlobalValue(construct_transcript_filename(make_local_transcript_filename(local_transcript_version())));
		return $local_transcript$.getGlobalValue();
	}
	...
	/**
	 * The master transcript file, containing operations from all communicating
	 * Cyc images.
	 */
	...
	public static final SubLObject set_master_transcript(SubLObject name) {
		...
		if ((NIL == transcript_server.use_transcript_server())) {
			$master_transcript$
					.setGlobalValue(construct_transcript_filename(name));
		}
		return $master_transcript$.getGlobalValue();
	}
	...
	public static final SubLObject make_master_transcript_filename(
			SubLObject version) {
		if ((version == UNPROVIDED)) {
			version = control_vars.kb_loaded();
		}
		{
			SubLObject filename = PrintLow.format(NIL, $str18$cyc_kb__a,
					version);
			return filename;
		}
	}
	...
	/**
	 * Produces the base of a transcript filename based on the cyc-image-id and
	 * a version number
	 */
	@SubL(source = "cycl/transcript-utilities.lisp", position = 11293)
	public static final SubLObject make_local_transcript_filename(
			SubLObject version_number) {
		if ((NIL == control_vars.cyc_image_id())) {
			control_vars.set_cyc_image_id();
		}
		{
			SubLObject filename = PrintLow.format(NIL, $str19$_a_local__a,
					control_vars.cyc_image_id(), version_number);
			return filename;
		}
	}
	...
	public static final SubLObject construct_transcript_filename(SubLObject name) {
		{
			SubLObject filename = Sequences.cconcatenate(
					transcript_directory(), new SubLObject[] { name, $str21$_,
							transcript_suffix() });
			return filename;
		}
	}
	...
	/**
	 * Returns a relative pathname to the current transcript directory, creating
	 * it if necessary.
	 */
	...
	public static final SubLObject transcript_directory_int(SubLObject kb_number) {
		{
			SubLObject directory = file_utilities.cyc_home_subdirectory(list(
					$str22$transcripts,
					PrintLow.format(NIL, $str23$_4__0D, kb_number)));
			if ((NIL == Filesys.directory_p(directory))) {
				file_utilities.make_directory_recursive(directory, T,
						UNPROVIDED);
			}
			return directory;
		}
	...
	public static final SubLObject declare_transcript_utilities_file() {
		// declareFunction(myName, "transcript_eval", "TRANSCRIPT-EVAL", 1, 1,
		// false);
		// declareFunction(myName, "transcript_form", "TRANSCRIPT-FORM", 1, 1,
		// false);
		// declareFunction(myName, "transcript_form_int", "TRANSCRIPT-FORM-INT",
		// 3, 0, false);
		// declareFunction(myName, "form_to_transcript_form",
		// "FORM-TO-TRANSCRIPT-FORM", 1, 0, false);
		declareFunction(myName, "initialize_transcript_handling",
				"INITIALIZE-TRANSCRIPT-HANDLING", 0, 0, false);
		declareFunction(myName, "transcript_suffix", "TRANSCRIPT-SUFFIX", 0, 0,
				false);
		// declareFunction(myName, "master_transcript_already_exists",
		// "MASTER-TRANSCRIPT-ALREADY-EXISTS", 0, 0, false);
		declareFunction(myName, "set_master_transcript_already_exists",
				"SET-MASTER-TRANSCRIPT-ALREADY-EXISTS", 1, 0, false);
		declareFunction(myName, "local_transcript_version",
				"LOCAL-TRANSCRIPT-VERSION", 0, 0, false);
		// declareFunction(myName, "inc_local_transcript_version",
		// "INC-LOCAL-TRANSCRIPT-VERSION", 0, 0, false);
		// declareFunction(myName, "local_transcript_history",
		// "LOCAL-TRANSCRIPT-HISTORY", 0, 0, false);
		// declareFunction(myName, "local_transcript_history_add",
		// "LOCAL-TRANSCRIPT-HISTORY-ADD", 1, 0, false);
		// declareFunction(myName, "local_hl_transcript_version",
		// "LOCAL-HL-TRANSCRIPT-VERSION", 0, 0, false);
		// declareFunction(myName, "inc_local_hl_transcript_version",
		// "INC-LOCAL-HL-TRANSCRIPT-VERSION", 0, 0, false);
		// declareFunction(myName, "local_hl_transcript_history",
		// "LOCAL-HL-TRANSCRIPT-HISTORY", 0, 0, false);
		// declareFunction(myName, "local_hl_transcript_history_add",
		// "LOCAL-HL-TRANSCRIPT-HISTORY-ADD", 1, 0, false);
		// declareFunction(myName, "read_transcript_position",
		// "READ-TRANSCRIPT-POSITION", 0, 0, false);
		// declareFunction(myName, "set_read_transcript_position",
		// "SET-READ-TRANSCRIPT-POSITION", 1, 0, false);
		// declareFunction(myName, "approx_chars_per_op", "APPROX-CHARS-PER-OP",
		// 0, 0, false);
		// declareFunction(myName, "set_approx_chars_per_op",
		// "SET-APPROX-CHARS-PER-OP", 1, 0, false);
		declareFunction(myName, "local_transcript", "LOCAL-TRANSCRIPT", 0, 0,
				false);
		// declareFunction(myName, "mark_local_transcript",
		// "MARK-LOCAL-TRANSCRIPT", 1, 0, false);
		// declareFunction(myName, "roll_local_transcript",
		// "ROLL-LOCAL-TRANSCRIPT", 0, 0, false);
		declareFunction(myName, "new_local_transcript", "NEW-LOCAL-TRANSCRIPT",
				0, 0, false);
		declareFunction(myName, "new_local_transcript_int",
				"NEW-LOCAL-TRANSCRIPT-INT", 0, 0, false);
		// declareFunction(myName, "read_transcript", "READ-TRANSCRIPT", 0, 0,
		// false);
		declareFunction(myName, "set_read_transcript", "SET-READ-TRANSCRIPT",
				1, 0, false);
		declareFunction(myName, "master_transcript", "MASTER-TRANSCRIPT", 0, 0,
				false);
		declareFunction(myName, "set_master_transcript",
				"SET-MASTER-TRANSCRIPT", 0, 1, false);
		// declareFunction(myName, "get_all_transcripts_image",
		// "GET-ALL-TRANSCRIPTS-IMAGE", 0, 0, false);
		// declareFunction(myName, "local_hl_transcript", "LOCAL-HL-TRANSCRIPT",
		// 0, 0, false);
		// declareFunction(myName, "mark_local_hl_transcript",
		// "MARK-LOCAL-HL-TRANSCRIPT", 1, 0, false);
		// declareFunction(myName, "roll_local_hl_transcript",
		// "ROLL-LOCAL-HL-TRANSCRIPT", 0, 0, false);
		// declareFunction(myName, "new_local_hl_transcript",
		// "NEW-LOCAL-HL-TRANSCRIPT", 0, 0, false);
		// declareFunction(myName, "new_local_hl_transcript_int",
		// "NEW-LOCAL-HL-TRANSCRIPT-INT", 0, 0, false);
		declareFunction(myName, "make_master_transcript_filename",
				"MAKE-MASTER-TRANSCRIPT-FILENAME", 0, 1, false);
		declareFunction(myName, "make_local_transcript_filename",
				"MAKE-LOCAL-TRANSCRIPT-FILENAME", 1, 0, false);
		// declareFunction(myName, "make_local_hl_transcript_filename",
		// "MAKE-LOCAL-HL-TRANSCRIPT-FILENAME", 1, 0, false);
		declareFunction(myName, "construct_transcript_filename",
				"CONSTRUCT-TRANSCRIPT-FILENAME", 1, 0, false);
		declareFunction(myName, "transcript_directory", "TRANSCRIPT-DIRECTORY",
				0, 0, false);
		// declareFunction(myName, "next_transcript_directory",
		// "NEXT-TRANSCRIPT-DIRECTORY", 0, 0, false);
		declareFunction(myName, "transcript_directory_int",
				"TRANSCRIPT-DIRECTORY-INT", 1, 0, false);
		// declareFunction(myName, "mark_transcript_filename",
		// "MARK-TRANSCRIPT-FILENAME", 2, 0, false);
		// declareFunction(myName, "get_count_ops_data", "GET-COUNT-OPS-DATA",
		// 1, 0, false);
		// declareFunction(myName, "get_current_op_count",
		// "GET-CURRENT-OP-COUNT", 1, 0, false);
		// declareFunction(myName, "get_current_position",
		// "GET-CURRENT-POSITION", 1, 0, false);
		// declareFunction(myName, "update_count_ops_data",
		// "UPDATE-COUNT-OPS-DATA", 3, 0, false);
		// declareFunction(myName, "really_count_ops", "REALLY-COUNT-OPS", 1, 0,
		// false);
		// declareFunction(myName, "count_operations", "COUNT-OPERATIONS", 1, 0,
		// false);
		// declareFunction(myName, "estimate_number_of_ops",
		// "ESTIMATE-NUMBER-OF-OPS", 1, 0, false);
		// declareFunction(myName, "collect_ops_in_range",
		// "COLLECT-OPS-IN-RANGE", 2, 2, false);
		// declareFunction(myName, "bp_count_ops", "BP-COUNT-OPS", 1, 0, false);
		// declareFunction(myName, "constant_modifications_in_transcript",
		// "CONSTANT-MODIFICATIONS-IN-TRANSCRIPT", 1, 0, false);
		// declareFunction(myName,
		// "report_constant_modifications_in_transcript",
		// "REPORT-CONSTANT-MODIFICATIONS-IN-TRANSCRIPT", 0, 2, false);
		// declareFunction(myName,
		// "report_constant_modifications_in_transcript_to_file",
		// "REPORT-CONSTANT-MODIFICATIONS-IN-TRANSCRIPT-TO-FILE", 1, 1, false);
		// declareFunction(myName, "encapsulated_cyclist_string",
		// "ENCAPSULATED-CYCLIST-STRING", 1, 0, false);
		// declareFunction(myName, "reset_transcript_rename_hash",
		// "RESET-TRANSCRIPT-RENAME-HASH", 0, 0, false);
		// declareFunction(myName, "sort_transcript_renames",
		// "SORT-TRANSCRIPT-RENAMES", 0, 0, false);
		// declareFunction(myName, "add_transcript_rename_info",
		// "ADD-TRANSCRIPT-RENAME-INFO", 6, 0, false);
		// declareFunction(myName, "rem_transcript_rename_info",
		// "REM-TRANSCRIPT-RENAME-INFO", 1, 0, false);
		// declareFunction(myName, "reset_transcript_create_hash",
		// "RESET-TRANSCRIPT-CREATE-HASH", 0, 0, false);
		// declareFunction(myName, "sort_transcript_creates",
		// "SORT-TRANSCRIPT-CREATES", 0, 0, false);
		// declareFunction(myName, "add_transcript_create_info",
		// "ADD-TRANSCRIPT-CREATE-INFO", 5, 0, false);
		// declareFunction(myName, "constant_created_in_transcript",
		// "CONSTANT-CREATED-IN-TRANSCRIPT", 1, 0, false);
		// declareFunction(myName, "rem_transcript_create_info",
		// "REM-TRANSCRIPT-CREATE-INFO", 1, 0, false);
		// declareFunction(myName, "write_specific_transcript_file_as_ke_text",
		// "WRITE-SPECIFIC-TRANSCRIPT-FILE-AS-KE-TEXT", 2, 0, false);
		// declareFunction(myName, "transcript_file_to_ke_text",
		// "TRANSCRIPT-FILE-TO-KE-TEXT", 1, 0, false);
		// declareFunction(myName, "unencapsulate_to_string",
		// "UNENCAPSULATE-TO-STRING", 1, 1, false);
		// declareFunction(myName, "unencapsulate_string",
		// "UNENCAPSULATE-STRING", 1, 0, false);
		return NIL;
	}

	public static final SubLObject init_transcript_utilities_file() {
		$transcript_suffix$ = defparameter("*TRANSCRIPT-SUFFIX*", $str2$ts);
		$master_transcript_already_exists$ = defparameter(
				"*MASTER-TRANSCRIPT-ALREADY-EXISTS*", NIL);
		$local_transcript_version$ = deflexical(
				"*LOCAL-TRANSCRIPT-VERSION*",
				((NIL != Symbols.boundp($sym3$_LOCAL_TRANSCRIPT_VERSION_)) ? ((SubLObject) $local_transcript_version$
						.getGlobalValue()) : ZERO_INTEGER));
		$local_transcript_history$ = deflexical(
				"*LOCAL-TRANSCRIPT-HISTORY*",
				((NIL != Symbols.boundp($sym4$_LOCAL_TRANSCRIPT_HISTORY_)) ? ((SubLObject) $local_transcript_history$
						.getGlobalValue()) : NIL));
		$local_hl_transcript_version$ = deflexical(
				"*LOCAL-HL-TRANSCRIPT-VERSION*",
				((NIL != Symbols.boundp($sym5$_LOCAL_HL_TRANSCRIPT_VERSION_)) ? ((SubLObject) $local_hl_transcript_version$
						.getGlobalValue()) : ZERO_INTEGER));
		$local_hl_transcript_history$ = deflexical(
				"*LOCAL-HL-TRANSCRIPT-HISTORY*",
				((NIL != Symbols.boundp($sym6$_LOCAL_HL_TRANSCRIPT_HISTORY_)) ? ((SubLObject) $local_hl_transcript_history$
						.getGlobalValue()) : NIL));
		$read_transcript_position$ = deflexical(
				"*READ-TRANSCRIPT-POSITION*",
				((NIL != Symbols.boundp($sym7$_READ_TRANSCRIPT_POSITION_)) ? ((SubLObject) $read_transcript_position$
						.getGlobalValue()) : ZERO_INTEGER));
		$approx_chars_per_op$ = defparameter("*APPROX-CHARS-PER-OP*", $int8$206);
		$local_transcript$ = deflexical(
				"*LOCAL-TRANSCRIPT*",
				((NIL != Symbols.boundp($sym9$_LOCAL_TRANSCRIPT_)) ? ((SubLObject) $local_transcript$
						.getGlobalValue()) : NIL));
		$read_transcript$ = deflexical(
				"*READ-TRANSCRIPT*",
				((NIL != Symbols.boundp($sym15$_READ_TRANSCRIPT_)) ? ((SubLObject) $read_transcript$
						.getGlobalValue()) : NIL));
		$master_transcript$ = deflexical(
				"*MASTER-TRANSCRIPT*",
				((NIL != Symbols.boundp($sym16$_MASTER_TRANSCRIPT_)) ? ((SubLObject) $master_transcript$
						.getGlobalValue()) : NIL));
		$local_hl_transcript$ = deflexical(
				"*LOCAL-HL-TRANSCRIPT*",
				((NIL != Symbols.boundp($sym17$_LOCAL_HL_TRANSCRIPT_)) ? ((SubLObject) $local_hl_transcript$
						.getGlobalValue()) : NIL));
		$count_ops_table$ = deflexical(
				"*COUNT-OPS-TABLE*",
				((NIL != Symbols.boundp($sym28$_COUNT_OPS_TABLE_)) ? ((SubLObject) $count_ops_table$
						.getGlobalValue()) : NIL));
		$transcript_rename_hash$ = deflexical(
				"*TRANSCRIPT-RENAME-HASH*",
				((NIL != Symbols.boundp($sym59$_TRANSCRIPT_RENAME_HASH_)) ? ((SubLObject) $transcript_rename_hash$
						.getGlobalValue()) : NIL));
		$transcript_create_hash$ = deflexical(
				"*TRANSCRIPT-CREATE-HASH*",
				((NIL != Symbols.boundp($sym63$_TRANSCRIPT_CREATE_HASH_)) ? ((SubLObject) $transcript_create_hash$
						.getGlobalValue()) : NIL));
		return NIL;
	}
	...
	public static final SubLObject setup_transcript_utilities_file() {
		...
		subl_macro_promotions
				.declare_defglobal($sym3$_LOCAL_TRANSCRIPT_VERSION_);
		subl_macro_promotions
				.declare_defglobal($sym4$_LOCAL_TRANSCRIPT_HISTORY_);
		subl_macro_promotions
				.declare_defglobal($sym5$_LOCAL_HL_TRANSCRIPT_VERSION_);
		subl_macro_promotions
				.declare_defglobal($sym6$_LOCAL_HL_TRANSCRIPT_HISTORY_);
		subl_macro_promotions
				.declare_defglobal($sym7$_READ_TRANSCRIPT_POSITION_);
		...
		subl_macro_promotions
				.declare_defglobal($sym59$_TRANSCRIPT_RENAME_HASH_);
		subl_macro_promotions
				.declare_defglobal($sym63$_TRANSCRIPT_CREATE_HASH_);
		utilities_macros.register_cyc_api_function(
				$sym66$WRITE_SPECIFIC_TRANSCRIPT_FILE_AS_KE_TEXT, $list67,
				$str68$Generate_a_KE_Text_file_from_a_tr, $list69, NIL);
		return NIL;
	}
	...
	public static final SubLSymbol $sym1$IGNORE_ERRORS_HANDLER = makeSymbol(
	...
	public static final SubLList $list39 = list(
			makeSymbol("ENCAPSULATED-CYCLIST"), makeSymbol("CYC-IMAGE-ID"),
			makeSymbol("WHEN"),
			cons(makeSymbol("FUNCTION"), makeSymbol("ARGS")));
	public static final SubLSymbol $sym40$FI_CREATE = makeSymbol("FI-CREATE");
	public static final SubLList $list41 = list(makeSymbol("NAME"),
			makeSymbol("EXTERNAL-ID"));
	public static final SubLSymbol $sym42$FI_RENAME = makeSymbol("FI-RENAME");
	public static final SubLList $list43 = list(makeSymbol("TERM"),
			makeSymbol("NEW-NAME"));
	...
	public static final SubLList $list51 = list(
			makeSymbol("TRANSCRIPT-CREATE-LIST"),
			makeSymbol("TRANSCRIPT-RENAME-LIST"),
			makeSymbol("TRANSCRIPT-KILL-LIST"));
	public static final SubLString $str52$_______S_KILLED = makeString("~%;;; ~S KILLED");
	public static final SubLList $list53 = list(makeSymbol("OP-NUM"),
			makeSymbol("EXTERNAL-ID"), makeSymbol("NAME"), makeSymbol("WHO"),
			makeSymbol("WHEN"));
	...
	public static final SubLList $list56 = list(makeSymbol("OP-NUM"),
			makeSymbol("EXTERNAL-ID"), makeSymbol("OLD-NAME"),
			makeSymbol("NEW-NAME"), makeSymbol("WHO"), makeSymbol("WHEN"));
	...
	public static final SubLList $list67 = list(
			makeSymbol("TRANSCRIPT-FILENAME"), makeSymbol("OUTPUT-FILENAME"));
	...
	public static final SubLList $list69 = list(
			list(makeSymbol("TRANSCRIPT-FILENAME"), makeSymbol("STRINGP")),
			list(makeSymbol("OUTPUT-FILENAME"), makeSymbol("STRINGP")));
	...
	public static final SubLList $list74 = list(makeSymbol("OLD-NAME"),
			makeSymbol("NEW-NAME"));
	...
	public static final SubLList $list78 = list(makeSymbol("KILL-FORT"),
			makeSymbol("KEEP-FORT"));
	...
	public static final SubLObject $const94$DefaultMonotonicPredicate = constant_handles
