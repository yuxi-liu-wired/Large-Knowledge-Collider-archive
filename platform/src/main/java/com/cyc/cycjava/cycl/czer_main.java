...
Also finds the nart version of a non-atomic mt before returning it, if one exists. */
    ...
    }
    {
      final SubLThread thread = SubLProcess.currentSubLThread();
      {
        ...
        for (var = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var.rest(), var = cdolist_list_var.first()) {
          thread.resetMultipleValues();
          {
            SubLObject referenced_as_list = standardize_variable_memory_binding(var,expression);
  ...
  @SubL(source = "cycl/czer-main.lisp", position = 97370) 
        ...
        if ((NIL == ambiguousP)) {
          {
            ...
            for (binding = csome_list_var.first(); (!(((NIL != ambiguousP)
                  || (NIL == csome_list_var)))); csome_list_var = csome_list_var.rest(), binding = csome_list_var.first()) {
              if ((NIL != list_utilities.simple_tree_findP(binding, expression))) {
                if (((NIL != referenced_as_list)
                    || (NIL != list_utilities.simple_tree_findP(var, expression)))) {
                  ambiguousP = T;
                }
  ...
  @SubL(source = "cycl/czer-main.lisp", position = 102681) 
