...
public  final class cycl_utilities extends SubLTranslatedFile {
  ...
  @SubL(source = "cycl/cycl-utilities.lisp", position = 50910) 
  public static final SubLObject expression_nsubst_free_vars(SubLObject v_new, SubLObject old, SubLObject expression, SubLObject test) {
    if ((test == UNPROVIDED)) {
  ...
  @SubL(source = "cycl/cycl-utilities.lisp", position = 48468) 
  public static final SubLObject expression_nsubst_free_vars_int(SubLObject v_new, SubLObject old, SubLObject expression, SubLObject test) {
    {
      final SubLThread thread = SubLProcess.currentSubLThread();
      if ((NIL != Functions.funcall(test, expression, old))) {
          ...
          SubLObject result = NIL;
          {
            ...
            try {
              ...
              if ((NIL != czer_vars.$canonicalize_variablesP$.getDynamicValue(thread))) {
                ...
                result = el_utilities.make_unary_formula(formula_arg0(expression), expression_nsubst_free_vars_int(v_new, old, formula_arg1(expression, UNPROVIDED), test));
              }
            } finally {
      ...
      } else if ((NIL != cycl_grammar.fast_quote_term_p(expression))) {
        {
          SubLObject result = NIL;
          {
            ...
            try {
              ...
              result = el_utilities.make_unary_formula($const27$Quote, expression_nsubst_free_vars_int(v_new, old, formula_arg1(expression, UNPROVIDED), test));
            } finally {
              czer_vars.$inside_quote$.rebind(_prev_bind_0, thread);
            }
          }
          return result;
           ...
           && (NIL != subl_promotions.memberP(old, formula_arg1(expression, UNPROVIDED), test, UNPROVIDED)))) {
        return el_utilities.make_binary_formula($const28$ExpandSubLFn, conses_high.subst(v_new, old, formula_arg1(expression, UNPROVIDED), test, UNPROVIDED), expression_nsubst_free_vars_int(v_new, old, formula_arg2(expression, UNPROVIDED), test));
      } else {
        {
          SubLObject seqvar = el_utilities.sequence_var(expression, UNPROVIDED);
          SubLObject substituted_seqvar = (((NIL != seqvar)
               && (NIL == opaque_seqvarP(expression))) ? ((SubLObject) expression_nsubst_free_vars_int(v_new, old, seqvar, test)) : seqvar);
  ...
  @SubL(source = "cycl/cycl-utilities.lisp", position = 3035) 
  public static final SubLObject opaque_seqvarP(SubLObject formula) {
    {
      final SubLThread thread = SubLProcess.currentSubLThread();
      return Functions.funcall($opaque_seqvar_function$.getDynamicValue(thread), formula);
    }
  }
