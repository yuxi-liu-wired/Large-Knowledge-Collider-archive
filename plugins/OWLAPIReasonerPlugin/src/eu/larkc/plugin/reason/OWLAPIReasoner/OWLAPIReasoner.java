...
public class OWLAPIReasoner {
		...
		private String reasonerID = "eu.larkc.plugin.reason.OWLAPIReasoner";
	
		
		public BooleanInformationSet sparqlAsk(SPARQLQuery theQuery, SetOfStatements statements) {
			//to be done later
			return null;
		}
...
//			System.out.println("[OWLAIReasoner] resultVars="+resultVars);
...
//		OWLQuery query = new OWLQuery();
	...
	//		System.out.println("[OWLAPIReasoner] currentVar = " + currentVar);
	    	
		BasicPattern newpattern = new BasicPattern();
		
		newpattern = OWLProcess.applyBinding2Pattern(var, value, pattern);	
		
	//	System.out.println("[OWLAPIReasoner]* newpattern = " + newpattern);
	...
	//	   System.out.println("[OWLAPIReasoner] existing pattern = " + pattern);
		   
		   Iterator<Triple>  iter3 = pattern.iterator();
		   while (iter3.hasNext()){
			   Triple t = iter3.next(); 
   //	 	   System.out.println("[OWLAPIReasoner] element = " + t + " has removed from the pattern ");
			   iter3.remove();
		 	   };
