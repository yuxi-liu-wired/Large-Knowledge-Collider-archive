...
public class KeywordReasonerPlugin3 extends Plugin {
	 ...
	 */
	public KeywordReasonerPlugin3(URI pluginUri) {
		super(pluginUri);
	...
	protected void initialiseInternal(SetOfStatements pluginArguments) {
		logger.info("KeywordReasonerPlugin3 initialized.");	
	}
	 ...
	 * 
	 * @author Gaston Tagni <g.e.tagni@vu.nl> and Zhisheng Huang <huang@cs.vu.nl>
	 * @version 1.0
	 */
	@Override
	protected SetOfStatements invokeInternal(SetOfStatements input) {
		logger.info("KeywordReasonerPlugin3 is working...");
...
//		SetOfStatements results = (SetOfStatements)KRUtils.wrapKeywordsIntoSetOfStatements(keywords, new URIImpl("http://www.vu.nl/larkc/keywordreasoner/results"));
		
			/* get the set of keywords as a set of converted keywords for input of workflow */
			
			SetOfStatements results = KRUtils.convertKeywordsIntoSetOfStatements(keywords);
			
			
			
			String pattern = KRUtils.convertSetOfStatementsIntoPattern(results);
			
			StringBuffer s = new StringBuffer(original);
			
			String newquery = s.insert(s.indexOf("{")+1, pattern).toString();
			
		   System.out.print("newquery ="+newquery);
			
		    SPARQLQuery output = DataFactory.INSTANCE.createSPARQLQuery(newquery);
			
	       results = output.toRDF();
		
		   logger.info("KeywordReasoner3 done.");		
